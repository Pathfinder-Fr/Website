...
namespace ScrewTurn.Wiki.Plugins.SqlCommon
{
    ...
    public interface IIndexConnector
    {
        ...
        int SaveDataForDocument(IDocument document, WordInfo[] content, WordInfo[] title, WordInfo[] keywords,
            object state);
    }
    ...
    /// <param name="state">A state object passed from the index.</param>
    /// <returns>The number of stored occurrences.</returns>
    public delegate int SaveDataForDocument(
        IDocument document, WordInfo[] content, WordInfo[] title, WordInfo[] keywords, object state);
