...
namespace ScrewTurn.Wiki.Plugins.SqlCommon
{
    ...
    public abstract class SqlFilesStorageProviderBase : SqlStorageProviderBase, IFilesStorageProviderV30
    {
        ...
        private bool DirectoryExists(IDbConnection connection, string directory)
        {
            ...
            var query = queryBuilder.SelectCountFrom("Directory");
        ...
        /// <returns><c>true</c> if the directory exists, <c>false</c> otherwise.</returns>
        /// <remarks>The root directory always exists.</remarks>
        private bool DirectoryExists(IDbTransaction transaction, string directory)
        {
            directory = PrepareDirectory(directory);
        ...
        private bool FileExists(IDbConnection connection, string fullName)
        {
            ...
            var query = queryBuilder.SelectCountFrom("File");
        ...
        /// <param name="fullName">The file full name, for example "/file.txt" or "/directory/file.txt".</param>
        /// <returns><c>true</c> if the file exists, <c>false</c> otherwise.</returns>
        private bool FileExists(IDbTransaction transaction, string fullName)
        {
            string directory, file;
            SplitFileFullName(fullName, out directory, out file);
        ...
        public IEnumerable<string> ListFiles(string directory)
        {
            ...
            var query = queryBuilder.SelectFrom("File", new[] {"Name"});
            ...
            if (reader != null)
            {
                ...
                while (reader.Read())
                {
                    result.Add(directory + reader["Name"]);
                }
        ...
        private IEnumerable<string> ListDirectories(IDbConnection connection, string directory)
        {
            directory = PrepareDirectory(directory);
            ...
            var query = queryBuilder.SelectFrom("Directory", new[] {"FullPath"});
        ...
        /// </param>
        /// <returns>The list of Directories in the Directory.</returns>
        private string[] ListDirectories(IDbTransaction transaction, string directory)
        {
            directory = PrepareDirectory(directory);
        ...
        public IEnumerable<string> ListDirectories(string directory)
        {
            ...
            var result = ListDirectories(connection, directory);
            CloseConnection(connection);
        ...
        public bool StoreFile(string fullName, Stream sourceStream, bool overwrite)
        {
            ...
            var fileExists = FileExists(transaction, fullName);
            ...
            byte[] fileData = null;
            var size = Tools.ReadStream(sourceStream, ref fileData, MaxFileSize);
            if (size < 0)
            {
                RollbackTransaction(transaction);
            ...
            if (fileExists)
            {
                query = queryBuilder.Update("File", new[] {"Size", "LastModified", "Data"},
                    new[] {"Size", "LastModified", "Data"});
                query = queryBuilder.Where(query, "Name", WhereOperator.Equals, "Name");
                query = queryBuilder.AndWhere(query, "Directory", WhereOperator.Equals, "Directory");
            ...
            {
                query = queryBuilder.InsertInto("File",
                    new[] {"Name", "Directory", "Size", "Downloads", "LastModified", "Data"},
                    new[] {"Name", "Directory", "Size", "Downloads", "LastModified", "Data"});
        ...
        /// <exception cref="ArgumentNullException">
        ///     If <paramref name="fullName" /> or <paramref name="destinationStream" /> are
        ///     <c>null</c>.
        /// </exception>
        /// <exception cref="ArgumentException">
        ///     If <paramref name="fullName" /> is empty or <paramref name="destinationStream" />
        ...
        public bool RetrieveFile(string fullName, Stream destinationStream, bool countHit)
        {
            ...
            var query = queryBuilder.SelectFrom("File", new[] {"Size", "Data"});
            query = queryBuilder.Where(query, "Name", WhereOperator.Equals, "Name");
            query = queryBuilder.AndWhere(query, "Directory", WhereOperator.Equals, "Directory");
            ...
            if (reader != null)
            {
                ...
                if (reader.Read())
                {
                    var read = ReadBinaryColumn(reader, "Data", destinationStream);
                    done = read == (long) reader["Size"];
                }
        ...
        public void SetFileRetrievalCount(string fullName, int count)
        {
            ...
            var query = queryBuilder.Update("File", new[] {"Downloads"}, new[] {"Downloads"});
            query = queryBuilder.Where(query, "Name", WhereOperator.Equals, "Name");
            query = queryBuilder.AndWhere(query, "Directory", WhereOperator.Equals, "Directory");
        ...
        public FileDetails GetFileDetails(string fullName)
        {
            ...
            var query = queryBuilder.SelectFrom("File", new[] {"Size", "Downloads", "LastModified"});
            query = queryBuilder.Where(query, "Name", WhereOperator.Equals, "Name");
            query = queryBuilder.AndWhere(query, "Directory", WhereOperator.Equals, "Directory");
        ...
        public bool DeleteFile(string fullName)
        {
            ...
            var query = queryBuilder.DeleteFrom("File");
            query = queryBuilder.Where(query, "Name", WhereOperator.Equals, "Name");
            query = queryBuilder.AndWhere(query, "Directory", WhereOperator.Equals, "Directory");
        ...
        public bool RenameFile(string oldFullName, string newFullName)
        {
            ...
            var query = queryBuilder.Update("File", new[] {"Name"}, new[] {"NewName"});
        ...
        public bool CreateDirectory(string path, string name)
        {
            ...
            var newDirectoryFullPath = PrepareDirectory(path + name);
            ...
            var query = QueryBuilder.NewQuery(builder)
                .InsertInto("Directory", new[] {"FullPath", "Parent"}, new[] {"FullPath", "Parent"});
        ...
        private bool DeleteDirectory(IDbTransaction transaction, string fullPath)
        {
            var dirs = ListDirectories(transaction, fullPath);
            ...
            var query = queryBuilder.DeleteFrom("Directory");
            query = queryBuilder.Where(query, "FullPath", WhereOperator.Equals, "FullPath");
        ...
        public bool DeleteDirectory(string fullPath)
        {
            ...
            var done = DeleteDirectory(transaction, fullPath);
        ...
        private bool RenameDirectory(IDbTransaction transaction, string oldFullPath, string newFullPath)
        {
            var directories = ListDirectories(transaction, oldFullPath);
            foreach (var dir in directories)
            {
                var trimmed = dir.Trim('/');
                var name = trimmed.Substring(trimmed.LastIndexOf("/") + 1);

                var newFullPathSub = PrepareDirectory(newFullPath + name);
            ...
            var query = queryBuilder.Update("Directory", new[] {"FullPath"}, new[] {"NewDirectory1"});
        ...
        public bool RenameDirectory(string oldFullPath, string newFullPath)
        {
            ...
            var done = RenameDirectory(transaction, oldFullPath, newFullPath);
        ...
        public IEnumerable<string> GetPagesWithAttachments()
        {
            var builder = CommandBuilder;
            var queryBuilder = new QueryBuilder(builder);

            var query = queryBuilder.SelectFrom("Attachment", new[] {"Page"});
        ...
        private IList<string> ListPageAttachments(IDbTransaction transaction, PageInfo pageInfo)
        {
            var builder = CommandBuilder;
            var queryBuilder = new QueryBuilder(builder);

            var query = queryBuilder.SelectFrom("Attachment", new[] {"Name"});
        ...
        /// <param name="pageInfo">The Page Info object that owns the Attachments.</param>
        /// <returns>The names, or an empty list.</returns>
        private IList<string> ListPageAttachments(IDbConnection connection, PageInfo pageInfo)
        {
            var builder = CommandBuilder;
            var queryBuilder = new QueryBuilder(builder);
        ...
        public IList<string> ListPageAttachments(PageInfo pageInfo)
        {
            ...
            var result = ListPageAttachments(connection, pageInfo);
            CloseConnection(connection);
        ...
        private bool AttachmentExists(IDbConnection connection, PageInfo page, string name)
        {
            var builder = CommandBuilder;
            var queryBuilder = new QueryBuilder(builder);

            var query = queryBuilder.SelectCountFrom("Attachment");
            query = queryBuilder.Where(query, "Name", WhereOperator.Equals, "Name");
        ...
        /// <param name="name">The attachment.</param>
        /// <returns><c>true</c> if the attachment exists, <c>false</c> otherwise.</returns>
        private bool AttachmentExists(IDbTransaction transaction, PageInfo page, string name)
        {
            var builder = CommandBuilder;
            var queryBuilder = new QueryBuilder(builder);
        ...
        public bool StorePageAttachment(PageInfo pageInfo, string name, Stream sourceStream, bool overwrite)
        {
            ...
            var attachmentExists = AttachmentExists(transaction, pageInfo, name);
            ...
            var size = Tools.ReadStream(sourceStream, ref attachmentData, MaxFileSize);
            if (size < 0)
            {
                RollbackTransaction(transaction);
                throw new ArgumentException("Source Stream contains too much data", "sourceStream");
            ...
            if (attachmentExists)
            {
                query = queryBuilder.Update("Attachment", new[] {"Size", "LastModified", "Data"},
                    new[] {"Size", "LastModified", "Data"});
                query = queryBuilder.Where(query, "Name", WhereOperator.Equals, "Name");
                query = queryBuilder.AndWhere(query, "Page", WhereOperator.Equals, "Page");
            ...
            {
                query = queryBuilder.InsertInto("Attachment",
                    new[] {"Name", "Page", "Size", "Downloads", "LastModified", "Data"},
                    new[] {"Name", "Page", "Size", "Downloads", "LastModified", "Data"});
        ...
        public bool RetrievePageAttachment(PageInfo pageInfo, string name, Stream destinationStream, bool countHit)
        {
            ...
            var query = queryBuilder.SelectFrom("Attachment", new[] {"Size", "Data"});
            query = queryBuilder.Where(query, "Name", WhereOperator.Equals, "Name");
            query = queryBuilder.AndWhere(query, "Page", WhereOperator.Equals, "Page");
        ...
        public void SetPageAttachmentRetrievalCount(PageInfo pageInfo, string name, int count)
        {
            ...
            var query = queryBuilder.Update("Attachment", new[] {"Downloads"}, new[] {"Downloads"});
            query = queryBuilder.Where(query, "Name", WhereOperator.Equals, "Name");
            query = queryBuilder.AndWhere(query, "Page", WhereOperator.Equals, "Page");
        ...
        public FileDetails GetPageAttachmentDetails(PageInfo pageInfo, string name)
        {
            ...
            var query = queryBuilder.SelectFrom("Attachment", new[] {"Size", "Downloads", "LastModified"});
            query = queryBuilder.Where(query, "Name", WhereOperator.Equals, "Name");
            query = queryBuilder.AndWhere(query, "Page", WhereOperator.Equals, "Page");
        ...
        public bool DeletePageAttachment(PageInfo pageInfo, string name)
        {
            ...
            var query = queryBuilder.DeleteFrom("Attachment");
            query = queryBuilder.Where(query, "Name", WhereOperator.Equals, "Name");
            query = queryBuilder.AndWhere(query, "Page", WhereOperator.Equals, "Page");
        ...
        public bool RenamePageAttachment(PageInfo pageInfo, string oldName, string newName)
        {
            ...
            var query = queryBuilder.Update("Attachment", new[] {"Name"}, new[] {"NewName"});
            query = queryBuilder.Where(query, "Name", WhereOperator.Equals, "OldName");
            query = queryBuilder.AndWhere(query, "Page", WhereOperator.Equals, "Page");
        ...
        public void NotifyPageRenaming(PageInfo oldPage, PageInfo newPage)
        {
            ...
            if (ListPageAttachments(transaction, newPage).Count> 0)
            {
                RollbackTransaction(transaction);
            ...
            var query = queryBuilder.Update("Attachment", new[] {"Page"}, new[] {"NewPage"});
