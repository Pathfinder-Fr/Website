...
namespace ScrewTurn.Wiki.Plugins.SqlServer
{
    ...
    public class SqlServerPagesStorageProvider : SqlPagesStorageProviderBase, IPagesStorageProviderV30
    {
        ...
        public override ComponentInformation Information { get; } =
            new ComponentInformation("SQL Server Pages Storage Provider", "Threeplicate Srl", "3.0.4.566",
                "http://www.screwturn.eu", "http://www.screwturn.eu/Version/SQLServerProv/Pages.txt");
        ...
        {
            var cmd = GetCommand(connString);
            cmd.CommandText = Resources.PagesDatabase;
        ...
        {
            var cmd = GetCommand(connString);
            cmd.CommandText = Resources.PagesDatabase_3000to3001;
...
exec sp_rename 'PagesProviderVersion', 'PagesProviderVersion_v2';";
            ...
            var pageStatus = new Dictionary<string, char>(500);

            cmd.CommandText = "select * from [Page_v2]";
            using (var reader = cmd.ExecuteReader())
            {
                var insertCmd = GetCommand(connString);
                insertCmd.CommandText =
                    "insert into [Page] ([Name], [Namespace], [CreationDateTime]) values (@Name, '', @CreationDateTime)";
            ...
            using (var reader = cmd.ExecuteReader())
            {
                var insertCmd = GetCommand(connString);
                insertCmd.CommandText =
                    "insert into [PageContent] ([Page], [Namespace], [Revision], [Title], [User], [LastModified], [Comment], [Content], [Description]) values (@Page, '', @Revision, @Title, @User, @LastModified, @Comment, @Content, NULL)";
                ...
                while (reader.Read())
                {
                    ...
                    insertCmd.Parameters.Add(new SqlParameter("@Comment", reader["Comment"] as string));
                        // Cannot be null in v2
            ...
            using (var reader = cmd.ExecuteReader())
            {
                var insertCmd = GetCommand(connString);
                insertCmd.CommandText =
                    "insert into [Message] ([Page], [Namespace], [Id], [Parent], [Username], [Subject], [DateTime], [Body]) values (@Page, '', @Id, @Parent, @Username, @Subject, @DateTime, @Body)";
            ...
            using (var reader = cmd.ExecuteReader())
            {
                var insertCmd = GetCommand(connString);
                insertCmd.CommandText =
                    "insert into [CategoryBinding] ([Namespace], [Category], [Page]) values ('', @Category, @Page)";
            ...
            using (var reader = cmd.ExecuteReader())
            {
                var insertCmd = GetCommand(connString);
                insertCmd.CommandText =
                    "insert into [NavigationPath] ([Name], [Namespace], [Page], [Number]) values (@Name, '', @Page, @Number)";
            ...
            var allPages = new List<string>(500);
            var allCategories = new List<string>(50);
            ...
            cmd.CommandText =
                "update [Page] set [Name] = @NewName where [Name] = @OldName; update [CategoryBinding] set [Page] = @NewName2 where [Page] = @OldName2";
            ...
            var keys = new string[pageStatus.Count];
            pageStatus.Keys.CopyTo(keys, 0);
            foreach (var key in keys)
            {
                if (key.Contains("."))
                {
                    var status = pageStatus[key];
            ...
            foreach (var pair in pageStatus)
            {
                if (pair.Value != 'N')
                {
