...
namespace ScrewTurn.Wiki.Plugins.SqlServer
{
    ...
    public class SqlServerSettingsStorageProvider : SqlSettingsStorageProviderBase
    {
        ...
        public override ComponentInformation Information { get; } =
            new ComponentInformation("SQL Server Settings Storage Provider", "Threeplicate Srl", "3.0.1.471",
                "http://www.screwturn.eu", "http://www.screwturn.eu/Version/SQLServerProv/Settings.txt");
        ...
        protected override void ValidateConnectionString(string connString)
        {
            try
            {
                using (var conn = CommandBuilder.GetConnection(connString))
                {
                    var cmd = CommandBuilder.GetCommand(conn, "select current_user", new List<Parameter>());
                }
            }
        ...
        {
            var cmd = GetCommand(connString);
            cmd.CommandText = Resources.SettingsDatabase;
