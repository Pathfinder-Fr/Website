// -----------------------------------------------------------------------
// <copyright file="WwwRewrite.cs" company="Pathfinder-fr.org">
...
namespace Sueetie.Core.HttpModules
{
    ...
    public class WwwRewrite : IHttpModule
    {
        ...
        {
            if (SiteSettings.Instance.HandleWwwSubdomain == "ignore" ||
                string.IsNullOrEmpty(SiteSettings.Instance.HandleWwwSubdomain))
            {
                return;
            }
            ...
            if (context.Request.PhysicalPath.EndsWith(".aspx", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (url.Contains("://www.") && SiteSettings.Instance.HandleWwwSubdomain == "remove")
                {
                    RemoveWww(context);
                }

                if (!url.Contains("://www.") && SiteSettings.Instance.HandleWwwSubdomain == "add")
                {
                    AddWww(context);
                }
            }
