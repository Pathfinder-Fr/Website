...
namespace ScrewTurn.Wiki.SearchEngine.Tests
{
    public abstract class IndexBaseTests : TestsBase
    {
        ...
        {
            var index = GetIndex();
            var imIndex = index as IInMemoryIndex;
            ...
            if (imIndex != null)
            {
                imIndex.IndexChanged += delegate { eventFired = true; };
            }
        ...
        public void StoreDocument()
        {
            ...
            if (imIndex != null)
            {
                imIndex.IndexChanged += delegate(object sender, IndexChangedEventArgs e)
                {
                    ...
                    if (eventFired)
                    {
                        ...
                        Assert.IsTrue(
                            e.ChangeData.Mappings.Find(
                                delegate(DumpedWordMapping m)
                                {
                                    return m.WordIndex == 0 && m.Location == WordLocation.Content.Location;
                                }) != null,
                            "Mappings does not contain a word");
                        Assert.IsTrue(
                            e.ChangeData.Mappings.Find(
                                delegate(DumpedWordMapping m)
                                {
                                    return m.WordIndex == 1 && m.Location == WordLocation.Content.Location;
                                }) != null,
                            "Mappings does not contain a word");
                        Assert.IsTrue(
                            e.ChangeData.Mappings.Find(
                                delegate(DumpedWordMapping m)
                                {
                                    return m.WordIndex == 2 && m.Location == WordLocation.Content.Location;
                                }) != null,
                            "Mappings does not contain a word");
                        Assert.IsTrue(
                            e.ChangeData.Mappings.Find(
                                delegate(DumpedWordMapping m)
                                {
                                    return m.WordIndex == 3 && m.Location == WordLocation.Content.Location;
                                }) != null,
                            "Mappings does not contain a word");
                        Assert.IsTrue(
                            e.ChangeData.Mappings.Find(
                                delegate(DumpedWordMapping m)
                                {
                                    return m.WordIndex == 0 && m.Location == WordLocation.Title.Location;
                                }) != null,
                            "Mappings does not contain a word");
                    }
                };
            ...
            Assert.AreEqual(5, index.StoreDocument(doc, null, PlainTextDocumentContent, null),
                "Wrong number of indexed words");
        ...
        public void StoreDocument_ExistingDocument()
        {
            ...
            Assert.AreEqual(7, index.StoreDocument(doc, null, PlainTextDocumentContent2, null),
                "Wrong number of indexed words");
        ...
        public void RemoveDocument()
        {
            ...
            var doc1 = MockDocument("Doc1", "Document 1", "ptdoc", DateTime.Now);
            var doc2 = MockDocument("Doc2", "Document 2", "ptdoc", DateTime.Now);
                    ...
                    if (eventFired)
                    {
                        ...
                        Assert.IsTrue(
                            e.ChangeData.Mappings.Find(
                                delegate(DumpedWordMapping m)
                                {
                                    return m.WordIndex == 1 && m.Location == WordLocation.Title.Location;
                                }) != null,
                            "Mappings does not contain a word");
                    }
                };
            ...
            var doc3 = MockDocument("Doc1", "Document 1", "ptdoc", DateTime.Now);
        ...
        public void Search_Basic()
        {
            ...
            var doc1 = MockDocument("Doc1", "Document 1", "ptdoc", DateTime.Now);
            var doc2 = MockDocument("Doc2", "Document 2", "ptdoc", DateTime.Now);
            var doc3 = MockDocument("Doc3", "Document 3", "ptdoc", DateTime.Now);
            ...
            var res1 = index.Search(new SearchParameters("specifications"));
            var res2 = index.Search(new SearchParameters("this"));
        ...
        public void Search_Basic_SingleResultWord()
        {
            ...
            var doc = MockDocument3("Doc", "Document", "ptdoc", DateTime.Now);

            if (imIndex != null) imIndex.IndexChanged += AutoHandlerForDocumentStorage;

            index.StoreDocument(doc, null, "", null);

            var res = index.Search(new SearchParameters("todo"));
        ...
        public void Search_Basic_MultipleWords()
        {
            ...
            var res = index.Search(new SearchParameters("this content"));
        ...
        public void Search_Filtered()
        {
            ...
            // The mocked document has a default content
            var doc1 = MockDocument("Doc1", "Document 1", "ptdoc", DateTime.Now);
            var doc2 = MockDocument("Doc2", "Document 2", "htmldoc", DateTime.Now);
            var doc3 = MockDocument("Doc3", "Document 3", "odoc", DateTime.Now);
            ...
            var res = index.Search(new SearchParameters("this", "ptdoc", "htmldoc"));
        ...
        public void Search_WithOptions_AtLeastOneWord()
        {
            ...
            var res = index.Search(new SearchParameters("this content", SearchOptions.AllWords));
        ...
        public void Search_WithOptions_AllWords(string query, int results)
        {
            ...
            var res = index.Search(new SearchParameters(query, SearchOptions.AllWords));
        ...
        public void Search_WithOptions_ExactPhrase(string query, int results)
        {
            ...
            var res = index.Search(new SearchParameters(query, SearchOptions.ExactPhrase));
        ...
        public void Search_ExactPhrase()
        {
            ...
            var doc = MockDocument4("Doc", "Document", "ptdoc", DateTime.Now);

            if (imIndex != null) imIndex.IndexChanged += AutoHandlerForDocumentStorage;

            index.StoreDocument(doc, null, "", null);

            var res = index.Search(new SearchParameters("content repeated content blah blah", SearchOptions.ExactPhrase));
        ...
        public void Search_Basic_Location()
        {
            ...
            index.StoreDocument(doc1, new[] {"development"}, "", null);

            var res = index.Search(new SearchParameters("document"));
            ...
            var doc2 = MockDocument2("Doc2", "Text 2", "ptdoc", DateTime.Now);
            index.StoreDocument(doc2, new[] {"document"}, "", null);
        ...
        public void Search_Basic_LocationRelevance_1()
        {
            ...
            var res = index.Search(new SearchParameters("dummy document"));
        ...
        public void Search_Basic_LocationRelevance_2()
        {
            ...
            index.StoreDocument(doc1, null, "", null);
            index.StoreDocument(doc2, new[] {"blah"}, "", null);

            // "blah" is only present in the keywords of doc2
            // "content" is only present in the content of doc1
            var res = index.Search(new SearchParameters("content blah"));
        ...
        public void Search_Basic_LocationRelevance_3()
        {
            ...
            // "blah" is only present in the keywords of doc2
            // "document" is only present in the title of doc1
            var res = index.Search(new SearchParameters("document blah"));
