...
namespace ScrewTurn.Wiki.ImportWiki
{
    ...
    public class Translator : ScrewTurn.Wiki.ImportWiki.ITranslator
    {
        ...
        public string Translate(string input)
        {
            ...
            var firsttitle = new Regex(@"(\={2,5}).+?\1");
            var doubleSquare = new Regex(@"\[{2}.+?\]{2}");
            var exlink = new Regex(@"(\[(\w+):\/\/([^\]]+)\])|(?<!\"")((?<Protocol>\w+):\/\/(?<Domain>[\w.]+\/?)\S*)(?!\"")");
            var mailto = new Regex(@"(\[mailto\:([\w\d\.\@]+)\])|(?<!\"")(mailto\:([\w\d\.\@]+))(?!\"")");
            var image = new Regex(@"\[Image\:.+?\]");
            var pre = new Regex(@"<pre>((.|\n|\r)*?</pre>)?");
            var newlinespace = new Regex(@"^\ .+", RegexOptions.Multiline);
            var transclusion = new Regex(@"(\{{2})(.|\n)+?(\}{2})");
            var math = new Regex(@"<math>(.|\n|\r)*?</math>");
            var references = new Regex(@"<ref>(.|\n|\r)*?</ref>");
            var references1 = new Regex(@"\<references\/\>");
            var redirect = new Regex(@"\#REDIRECT\ \[{2}.+\]{2}");
            Match match;
            ...
            while (match.Success)
            {
                ...
                if (IsNoWikied(match.Index, noWikiBegin, noWikiEnd, out end))
                {
                    var s = "{{{{<nowiki>" + match.Value.Replace("<pre>", "").Replace("</pre>", "") + @"</nowiki>}}}}";
            ...
            while (match.Success)
            {
                ...
                if (IsNoWikied(match.Index, noWikiBegin, noWikiEnd, out end))
                {
                    var str = match.Value.Replace(@"mailto:", "");
                    if (str[0] == '[')
                    {
                        var split = str.Split(new char[] { ' ' }, 2);
                        if (split.Length == 2)
                        {
                            sb.Remove(match.Index, match.Length);
            ...
            while (match.Success)
            {
                ...
                if (IsNoWikied(match.Index, noWikiBegin, noWikiEnd, out end))
                {
                    var str = match.Value.Remove(0, 7);
