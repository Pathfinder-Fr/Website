    ...
    public static class ProviderLoader
    {
        ...
        private static void VerifyConstraints<T>(T provider)
        {
            if (typeof(T) == typeof(IUsersStorageProviderV30))
            {
                ...
                var actualInstance = (IUsersStorageProviderV30)provider;
            ...
            ProviderCollector<T> collectorDisabled) where T : class, IProviderV30
        {
            ...
            var enabled = !IsDisabled(instance.GetType().FullName);
            try
            {
                if (enabled)
                {
        ...
        {
            var pluginAssemblies = Settings.Provider.ListPluginAssemblies();

            var users = new List<IUsersStorageProviderV30>(2);
            var dUsers = new List<IUsersStorageProviderV30>(2);
            var pages = new List<IPagesStorageProviderV30>(2);
            var dPages = new List<IPagesStorageProviderV30>(2);
            var files = new List<IFilesStorageProviderV30>(2);
            var dFiles = new List<IFilesStorageProviderV30>(2);
            var forms = new List<IFormatterProviderV30>(2);
            var dForms = new List<IFormatterProviderV30>(2);
            var cache = new List<ICacheProviderV30>(2);
            var dCache = new List<ICacheProviderV30>(2);

            for (var i = 0; i < pluginAssemblies.Length; i++)
            {
            ...
            out IFilesStorageProviderV30[] files, out IFormatterProviderV30[] formatters, out ICacheProviderV30[] cache)
        {
            ...
            var urs = new List<IUsersStorageProviderV30>();
            var pgs = new List<IPagesStorageProviderV30>();
            var fls = new List<IFilesStorageProviderV30>();
            var frs = new List<IFormatterProviderV30>();
            var che = new List<ICacheProviderV30>();
        ...
        public static ISettingsStorageProviderV30[] LoadAllSettingsStorageProviders(ISettingsStorageProviderV30 repository)
        {
            ...
            var result = new List<ISettingsStorageProviderV30>();

            foreach (var dll in repository.ListPluginAssemblies())
            {
                var asmBin = repository.RetrievePluginAssembly(dll);
                var asm = Assembly.Load(asmBin);
                ...
                foreach (Type type in types)
                {
                    ...
                    foreach (Type iface in interfaces)
                    {
                        if (iface == typeof(ISettingsStorageProviderV30))
                        {
                            try
                            {
                                var temp = asm.CreateInstance(type.ToString()) as ISettingsStorageProviderV30;
