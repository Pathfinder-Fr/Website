...
namespace ScrewTurn.Wiki
{
    ...
    public static class Tools
    {
        ...
        public static string GetJavaScriptIncludes()
        {
            ...
            foreach (var js in Directory.GetFiles(Settings.JsDirectory, "*.js"))
            {
        ...
        public static string BytesToString(long bytes)
        {
            ...
            else if (bytes < 1048576) return string.Format("{0:N2} KB", bytes / 1024F);
            else if (bytes < 1073741824) return string.Format("{0:N2} MB", bytes / 1048576F);
            else return string.Format("{0:N2} GB", bytes / 1073741824F);
        }
        ...
        public static string[] AvailableThemes
        {
            get
            {
                var dirs = Directory.GetDirectories(Settings.ThemesDirectory);
                var res = new string[dirs.Length];
        ...
        public static string[] AvailableCultures
        {
            get
            {
                ...
                var dllFiles = Directory.GetFiles(Path.Combine(Settings.RootDirectory, "bin"), "ScrewTurn.Wiki.resources.dll", SearchOption.AllDirectories);
                // List to collect constructed culture names
                var cultureNames = new List<string>();

                // Manually add en-US culture
                var enCI = new CultureInfo("en-US");
                ...
                foreach (var s in dllFiles)
                {
                    try
                    {
                        ...
                        var find = fullName.IndexOf("Culture=");
                        ...
                        var ci = new CultureInfo(fullName);
                        // StringBuilders
        ...
        {
            var result = span.Days.ToString() + "d ";
        ...
        {
            var sb = new StringBuilder(System.Text.RegularExpressions.Regex.Replace(html, "<[^>]*>", " "));
            sb.Replace("&nbsp;", " ");
            sb.Replace("  ", " ");
            return sb.ToString();
        }
        ...
        {
            var nspace = HttpContext.Current.Request["NS"] != null ? HttpContext.Current.Request["NS"] : "";
            var page = HttpContext.Current.Request["Page"] != null ? HttpContext.Current.Request["Page"] : "";
        ...
        public static string GetDirectoryName(string filename)
        {
            if (filename != null)
            {
                ...
                if (index > 0)
                {
                    var directoryName = filename.Substring(0, index + 1);
        ...
        public static UpdateStatus GetUpdateStatus(string url, string currentVersion, out string newVersion, out string newAssemblyUrl)
        {
            ...
            var urlHash = "UpdUrlCache-" + url.GetHashCode().ToString();
            ...
            try
            {
                ...
                var lines = ver.Replace("\r", "").Split(new char[] { '\n' }, StringSplitOptions.RemoveEmptyEntries);
