...
namespace ScrewTurn.Wiki
{
    ...
    public class IndexStorer : IndexStorerBase
    {
        ...
        protected override void LoadIndexInternal(out DumpedDocument[] documents, out DumpedWord[] words, out DumpedWordMapping[] mappings)
        {
            ...
            // 1. Load Documents
            using (var fs = new FileStream(documentsFile, FileMode.Open, FileAccess.Read, FileShare.None))
            {
                var count = ReadCount(fs);
            ...
            // 2. Load Words
            using (var fs = new FileStream(wordsFile, FileMode.Open, FileAccess.Read, FileShare.None))
            {
                var count = ReadCount(fs);
                var reader = new BinaryReader(fs, Encoding.UTF8);
            ...
            // 3. Load Mappings
            using (var fs = new FileStream(mappingsFile, FileMode.Open, FileAccess.Read, FileShare.None))
            {
                var count = ReadCount(fs);
                var reader = new BinaryReader(fs, Encoding.UTF8);
        ...
        {
            var allEqual = true;
            for (var i = 0; i < ReservedBytes.Length; i++)
            {
        ...
        private void InitFiles()
        {
            ...
            {
                using (var fs = new FileStream(documentsFile, FileMode.Create, FileAccess.Write, FileShare.None))
                {
            ...
            {
                using (var fs = new FileStream(wordsFile, FileMode.Create, FileAccess.Write, FileShare.None))
                {
                    var writer = new BinaryWriter(fs, Encoding.UTF8);
                    WriteHeader(writer);
                }
            ...
            {
                using (var fs = new FileStream(mappingsFile, FileMode.Create, FileAccess.Write, FileShare.None))
                {
                    var writer = new BinaryWriter(fs, Encoding.UTF8);
                    WriteHeader(writer);
                }
        ...
        {
            var result = new IndexStorerResult(null, null);

            // 1. Save Document
            using (var fs = new FileStream(documentsFile, FileMode.Open, FileAccess.ReadWrite, FileShare.None))
            {
                var count = ReadCount(fs);
            ...
            using (var fs = new FileStream(wordsFile, FileMode.Open, FileAccess.ReadWrite, FileShare.None))
            {
                var count = ReadCount(fs);
            ...
            // 3. Save Mappings
            using (var fs = new FileStream(mappingsFile, FileMode.Open, FileAccess.ReadWrite, FileShare.None))
            {
                var count = ReadCount(fs);
        ...
        private static string GetTempFile(string file)
        {
            ...
            var name = Path.GetFileNameWithoutExtension(file) + "_Temp" + Path.GetExtension(file);
        ...
        protected override void DeleteData(DumpedChange data, object state)
        {
            ...
            var tempDocumentsFile = GetTempFile(documentsFile);
            var tempWordsFile = GetTempFile(wordsFile);
            var tempMappingsFile = GetTempFile(mappingsFile);

            // 1. Remove Mappings
            using (var fsi = new FileStream(mappingsFile, FileMode.Open, FileAccess.Read, FileShare.None))
            {
                var count = ReadCount(fsi);
                var countLocation = (int)fsi.Position - 4;
                var writeCount = 0;
                var reader = new BinaryReader(fsi, Encoding.UTF8);
                using (var fso = new FileStream(tempMappingsFile, FileMode.Create, FileAccess.Write, FileShare.None))
                {
                    var writer = new BinaryWriter(fso, Encoding.UTF8);
                    WriteHeader(writer);
                    DumpedWordMapping m;
                    for (var i = 0; i < count; i++)
                    {
            ...
            // 2. Remove Words
            using (var fsi = new FileStream(wordsFile, FileMode.Open, FileAccess.Read, FileShare.None))
            {
                var count = ReadCount(fsi);
                var countLocation = (int)fsi.Position - 4;
                var writeCount = 0;
                var reader = new BinaryReader(fsi, Encoding.UTF8);
                using (var fso = new FileStream(tempWordsFile, FileMode.Create, FileAccess.Write, FileShare.None))
                {
                    var writer = new BinaryWriter(fso, Encoding.UTF8);
                    WriteHeader(writer);
                    DumpedWord w;
            ...
            // 3. Remove Document
            using (var fsi = new FileStream(documentsFile, FileMode.Open, FileAccess.Read, FileShare.None))
            {
                var count = ReadCount(fsi);
                var countLocation = (int)fsi.Position - 4;
                var reader = new BinaryReader(fsi, Encoding.UTF8);
                using (var fso = new FileStream(tempDocumentsFile, FileMode.Create, FileAccess.Write, FileShare.None))
                {
                    var writer = new BinaryWriter(fso, Encoding.UTF8);
                    WriteHeader(writer);
                    DumpedDocument d;
