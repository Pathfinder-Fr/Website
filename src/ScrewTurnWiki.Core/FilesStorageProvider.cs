...
namespace ScrewTurn.Wiki
{
    ...
    public class FilesStorageProvider : ProviderBase, IFilesStorageProviderV30
    {
        ...
        private string CheckPath(string path, string begin)
        {
            ...
            if (path.Length == begin.Length)
                return path;

            var end = path.Substring(begin.Length);

            if (end.Contains(Path.DirectorySeparatorChar + "..") || end.Contains(".." + Path.DirectorySeparatorChar))
            {
                throw new InvalidOperationException();
            }
        ...
        private string BuildFullPath(string partialPath)
        {
            ...
            var up = Path.Combine(GetDataDirectory(host), UploadDirectory);
        ...
        private string BuildFullPathForAttachments(string partialPath)
        {
            if (partialPath == null) partialPath = "";
            partialPath = partialPath.Replace("/", Path.DirectorySeparatorChar.ToString()).TrimStart(Path.DirectorySeparatorChar);
            var up = Path.Combine(GetDataDirectory(host), AttachmentsDirectory);
            return CheckPath(Path.Combine(up, partialPath), up); // partialPath CANNOT start with "\" -> Path.Combine does not work
        }
        ...
        public IEnumerable<string> ListDirectories(string directory)
        {
            ...
            var temp = Directory.GetDirectories(d);
        ...
        public bool StoreFile(string fullName, Stream sourceStream, bool overwrite)
        {
            ...
            var filename = BuildFullPath(fullName);
        ...
        private void AddDownloadHit(string itemName, string outputFile)
        {
            lock (this)
            {
                ...
                var lowercaseItemName = itemName.ToLowerInvariant();
        ...
        private void RenameDownloadHitsItem(string oldItemName, string newItemName, string outputFile)
        {
            lock (this)
            {
                ...
                var lowercaseOldItemName = oldItemName.ToLowerInvariant();
        ...
        private int GetFileRetrievalCount(string fullName)
        {
            ...
            lock (this)
            {
                ...
                var lines = File.ReadAllLines(GetFullPath(FileDownloadsFile));
        ...
        public bool RenameFile(string oldFullName, string newFullName)
        {
            ...
            var oldFilename = BuildFullPath(oldFullName);
            var newFilename = BuildFullPath(newFullName);
        ...
        public bool CreateDirectory(string path, string name)
        {
            ...
            var partialPath = path + (!path.EndsWith("/") ? "/" : "") + name;
            var d = BuildFullPath(partialPath);
        ...
        public bool RenameDirectory(string oldFullPath, string newFullPath)
        {
            ...
            var olddir = BuildFullPath(oldFullPath);
            var newdir = BuildFullPath(newFullPath);
        ...
        {
            return Directory
                .GetDirectories(GetFullPath(AttachmentsDirectory))
        ...
        public IList<string> ListPageAttachments(PageInfo pageInfo)
        {
            ...
            var dir = BuildFullPathForAttachments(GetPageAttachmentDirectory(pageInfo));
        ...
        public bool StorePageAttachment(PageInfo pageInfo, string name, Stream sourceStream, bool overwrite)
        {
            ...
            var filename = BuildFullPathForAttachments(GetPageAttachmentDirectory(pageInfo) + "/" + name);
        ...
        private int GetPageAttachmentRetrievalCount(PageInfo pageInfo, string name)
        {
            ...
            lock (this)
            {
                ...
                var lines = File.ReadAllLines(GetFullPath(AttachmentDownloadsFile));

                var lowercaseFullName = pageInfo.FullName + "." + name;
        ...
        public bool RenamePageAttachment(PageInfo pageInfo, string oldName, string newName)
        {
            ...
            var oldFilename = BuildFullPathForAttachments(d + "/" + oldName);
            if (!File.Exists(oldFilename)) throw new ArgumentException("Attachment does not exist", "oldName");

            var newFilename = BuildFullPathForAttachments(d + "/" + newName);
        ...
        public void NotifyPageRenaming(PageInfo oldPage, PageInfo newPage)
        {
            ...
            var oldName = GetPageAttachmentDirectory(oldPage);
            var newName = GetPageAttachmentDirectory(newPage);

            var oldDir = BuildFullPathForAttachments(oldName);
            var newDir = BuildFullPathForAttachments(newName);
