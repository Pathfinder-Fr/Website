...
namespace ScrewTurn.Wiki
{
    ...
    public static class Users
    {
        ...
        {
            var allUsers = new List<UserInfo>(1000);
        ...
        public static UserInfo FindUser(string username)
        {
            ...
            temp = null;
            var providers = Collectors.UsersProviderCollector.AllProviders;
            foreach (IUsersStorageProviderV30 p in providers)
            {
                var extProv = p as IUsersStorageProviderV30;
        ...
        public static bool SetUserData(UserInfo user, string key, string data)
        {
            ...
            var done = user.Provider.StoreUserData(user, key, data);
        ...
        public static bool RemoveUser(UserInfo user)
        {
            ...
            var done = user.Provider.RemoveUser(user);
            if (done)
            {
        ...
        {
            var directories = new List<string>(50);
            directories.Add("/");
        ...
        {
            foreach (var dir in provider.ListDirectories(current))
            {
                output.Add(dir);
        ...
        private static void RemovePermissions(UserInfo user)
        {
            ...
            {
                foreach (var dir in ListDirectories(prov))
                {
        ...
        {
            var mainLink = Settings.MainUrl + "Login.aspx?ResetCode=" + Tools.ComputeSecurityHash(username, email, dateTime) + "&Username=" + Tools.UrlEncode(username);
            var body = Settings.Provider.GetMetaDataItem(MetaDataItem.PasswordResetProcedureMessage, null).Replace("##USERNAME##",
        ...
        {
            var result = new List<UserGroup>(50);
        ...
        {
            var allGroups = user.Provider.GetUserGroups();

            var result = new List<UserGroup>();

            foreach (UserGroup group in allGroups)
            {
                if(user.Groups.Contains(group.Name, StringComparer.OrdinalIgnoreCase))
                {
                    result.Add(group);
                }
            }
        ...
        public static UserGroup FindUserGroup(string name)
        {
            ...
            return allGroups.FirstOrDefault(ug => StringComparer.OrdinalIgnoreCase.Equals(ug.Name, name));
        }
        ...
        public static bool RemoveUserGroup(UserGroup group)
        {
            ...
            var done = group.Provider.RemoveUserGroup(group);
        ...
        public static void NotifyLogout(string username)
        {
            ...
            if (user != null)
            {
                var prov = user.Provider as IUsersStorageProviderV30;
        ...
        public static bool SetEmailNotification(UserInfo user, PageInfo page, bool pageChanges, bool discussionMessages)
        {
            ...
            var pageChangeData = user.Provider.RetrieveUserData(user, PageChangesKey);
            var discussionMessagesData = user.Provider.RetrieveUserData(user, DiscussionMessagesKey);

            if (pageChangeData == null) pageChangeData = "";
            if (discussionMessagesData == null) discussionMessagesData = "";

            var pageChangesEntries = pageChangeData.Split(new char[] { ':' }, StringSplitOptions.RemoveEmptyEntries);
            var discussionMessagesEntries = discussionMessagesData.Split(new char[] { ':' }, StringSplitOptions.RemoveEmptyEntries);

            var pageChangesResult = new List<string>(pageChangesEntries.Length + 1);
            var discussionMessagesResult = new List<string>(discussionMessagesEntries.Length + 1);

            var lowercasePage = page.FullName.ToLowerInvariant();

            var added = false;
            foreach (var entry in pageChangesEntries)
            {
            ...
            added = false;
            foreach (var entry in discussionMessagesEntries)
            {
                if (entry.ToLowerInvariant() == lowercasePage)
                {
            ...
            var newPageChangesData = string.Join(":", pageChangesResult.ToArray());
            var newDiscussionMessagesData = string.Join(":", discussionMessagesResult.ToArray());

            var done = user.Provider.StoreUserData(user, PageChangesKey, newPageChangesData) &
        ...
        public static bool SetEmailNotification(UserInfo user, NamespaceInfo nspace, bool pageChanges, bool discussionMessages)
        {
            ...
            var pageChangeData = user.Provider.RetrieveUserData(user, NamespacePageChangesKey);
            var discussionMessagesData = user.Provider.RetrieveUserData(user, NamespaceDiscussionMessagesKey);
            ...
            var namespaceName = nspace != null ? nspace.Name : "<root>";
            var lowercaseNamespace = nspace != null ? nspace.Name.ToLowerInvariant() : "<root>";
            ...
            var done = user.Provider.StoreUserData(user, NamespacePageChangesKey, newPageChangesData) &
        ...
        public static void GetEmailNotification(UserInfo user, PageInfo page, out bool pageChanges, out bool discussionMessages)
        {
            ...
            var pageChangeEntries = pageChangeData.Split(new char[] { ':' }, StringSplitOptions.RemoveEmptyEntries);
            var discussionMessagesEntries = discussionMessagesData.Split(new char[] { ':' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        public static void GetEmailNotification(UserInfo user, NamespaceInfo nspace, out bool pageChanges, out bool discussionMessages)
        {
            ...
            var lowercaseNamespaces = nspace != null ? nspace.Name.ToLowerInvariant() : "<root>";

            var pageChangesData = user.Provider.RetrieveUserData(user, NamespacePageChangesKey);
            var discussionMessagesData = user.Provider.RetrieveUserData(user, NamespaceDiscussionMessagesKey);
            ...
            var pageChangeEntries = pageChangesData.Split(new char[] { ':' }, StringSplitOptions.RemoveEmptyEntries);
            var discussionMessagesEntries = discussionMessagesData.Split(new char[] { ':' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        public static UserInfo[] GetUsersToNotifyForPageChange(PageInfo page)
        {
            ...
            var result = new List<UserInfo>(temp.Length);
        ...
        {
            var result = new List<UserInfo>(array1.Length + array2.Length);
        ...
        {
            var result = new List<UserInfo>(200);
