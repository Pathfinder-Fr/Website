...
namespace ScrewTurn.Wiki
{
    ...
    public static class StartupTools
    {
        ...
        {
            var config = WebConfigurationManager.AppSettings["SettingsStorageProviderConfig"];
        ...
        private static void UpdateDllsIntoSettingsProvider(ISettingsStorageProviderV30 provider, string settingsProviderAsmName)
        {
            ...
            var fullPath = Path.Combine(Settings.PublicDirectory, "Plugins");

            if (!Directory.Exists(fullPath)) return;

            var dlls = Directory.GetFiles(fullPath, "*.dll");
            var installedDlls = provider.ListPluginAssemblies();

            foreach (var dll in dlls)
            {
                var found = false;
                var filename = Path.GetFileName(dll);
                foreach (var instDll in installedDlls)
                {
        ...
        public static void Startup()
        {
            ...
            var f = new FilesStorageProvider();
            ...
            var u = new UsersStorageProvider();
            ...
            var groupsCreated = VerifyAndCreateDefaultGroups();

            var p = new PagesStorageProvider();
            ...
            System.Threading.ThreadPool.QueueUserWorkItem(state =>
            {
                using (((WindowsIdentity)state).Impersonate())
                {
                    if ((DateTime.Now - Settings.LastPageIndexing).TotalDays > 7)
                    {
                        ...
                        {
                            using (var wr = new System.IO.StreamWriter(ms))
                            {
