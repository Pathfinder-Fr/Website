...
namespace ScrewTurn.Wiki
{
    ...
    public static class AuthWriter
    {
        ...
        private static bool SetPermissionForGlobals(AuthStatus status, string action, string subject)
        {
            ...
            {
                var done = SettingsProvider.AclManager.DeleteEntry(Actions.ForGlobals.ResourceMasterPrefix, action, subject);
            ...
            {
                var done = SettingsProvider.AclManager.StoreEntry(Actions.ForGlobals.ResourceMasterPrefix,
        ...
        private static bool SetPermissionForNamespace(AuthStatus status, NamespaceInfo nspace, string action, string subject)
        {
            ...
            if (status == AuthStatus.Delete)
            {
                var done = SettingsProvider.AclManager.DeleteEntry(Actions.ForNamespaces.ResourceMasterPrefix + namespaceName,
                    action, subject);
            ...
            {
                var done = SettingsProvider.AclManager.StoreEntry(Actions.ForNamespaces.ResourceMasterPrefix + namespaceName,
                    action, subject, status == AuthStatus.Grant ? Value.Grant : Value.Deny);
        ...
        private static bool SetPermissionForDirectory(AuthStatus status, IFilesStorageProviderV30 provider, string directory, string action, string subject)
        {
            ...
            var directoryName = AuthTools.GetDirectoryName(provider, directory);

            if (status == AuthStatus.Delete)
            {
                var done = SettingsProvider.AclManager.DeleteEntry(Actions.ForDirectories.ResourceMasterPrefix + directoryName,
                    action, subject);
            ...
            {
                var done = SettingsProvider.AclManager.StoreEntry(Actions.ForDirectories.ResourceMasterPrefix + directoryName,
                    action, subject, status == AuthStatus.Grant ? Value.Grant : Value.Deny);
        ...
        private static bool SetPermissionForPage(AuthStatus status, PageInfo page, string action, string subject)
        {
            ...
            if (status == AuthStatus.Delete)
            {
                var done = SettingsProvider.AclManager.DeleteEntry(Actions.ForPages.ResourceMasterPrefix + page.FullName,
                    action, subject);
            ...
            {
                var done = SettingsProvider.AclManager.StoreEntry(Actions.ForPages.ResourceMasterPrefix + page.FullName,
                    action, subject, status == AuthStatus.Grant ? Value.Grant : Value.Deny);
        ...
        private static bool RemoveEntriesForGlobals(string subject)
        {
            ...
            foreach (AclEntry entry in entries)
            {
                if (entry.Resource == Actions.ForGlobals.ResourceMasterPrefix)
                {
                    ...
                    var done = SetPermissionForGlobals(AuthStatus.Delete, entry.Action, subject);
        ...
        private static bool RemoveEntriesForNamespace(string subject, NamespaceInfo nspace)
        {
            ...
            foreach (AclEntry entry in entries)
            {
                ...
                {
                    // This call automatically logs the operation result
                    var done = SetPermissionForNamespace(AuthStatus.Delete, nspace, entry.Action, subject);
                    if (!done) return false;
                }
            }
        ...
        private static bool RemoveEntriesForPage(string subject, PageInfo page)
        {
            ...
            foreach (AclEntry entry in entries)
            {
                ...
                {
                    // This call automatically logs the operation result
                    var done = SetPermissionForPage(AuthStatus.Delete, page, entry.Action, subject);
                    if (!done) return false;
                }
            }
        ...
        private static bool RemoveEntriesForDirectory(string subject, IFilesStorageProviderV30 provider, string directory)
        {
            ...
            foreach (AclEntry entry in entries)
            {
                ...
                {
                    // This call automatically logs the operation result
                    var done = SetPermissionForDirectory(AuthStatus.Delete, provider, directory, entry.Action, subject);
                    if (!done) return false;
                }
            }
        ...
        public static void ClearEntriesForPage(string page)
        {
            ...
            var resourceName = Actions.ForPages.ResourceMasterPrefix + page;
