...
namespace ScrewTurn.Wiki
{
    ...
    public static class Pages
    {
        ...
        public static bool RemoveNamespace(NamespaceInfo nspace)
        {
            ...
            var done = realNspace.Provider.RemoveNamespace(realNspace);
            if (done)
            {
        ...
        private static void DeleteAllAttachments(List<PageInfo> pages)
        {
            foreach (IFilesStorageProviderV30 prov in Collectors.FilesProviderCollector.AllProviders)
            {
                ...
                {
                    var attachments = prov.ListPageAttachments(page);
        ...
        public static bool RenameNamespace(NamespaceInfo nspace, string newName)
        {
            ...
            var pageNames = new List<string>(pages.Count);
        ...
        private static void NotifyFilesProvidersForNamespaceRename(List<string> pages, string nspace, string newName)
        {
            foreach (IFilesStorageProviderV30 prov in Collectors.FilesProviderCollector.AllProviders)
            {
                ...
                {
                    var pageToNotify = new PageInfo(NameTools.GetFullName(nspace, page), null, DateTime.Now);
                    var newPage = new PageInfo(NameTools.GetFullName(newName, page), null, DateTime.Now);
        ...
        public static List<IList<PageInfo>> FindPages(FindPagesFilter filter)
        {
            //return new List<PageInfo[]>(new[] { Collectors.PagesProviderCollector.GetProvider(Settings.DefaultPagesProvider).FindPages(filter) });
            return Collectors.PagesProviderCollector.AllProviders.Select(s => s.FindPages(filter)).ToList();
        }
        ...
        {
            var temp = page.Provider.GetBackups(page);
        ...
        public static bool DeleteBackups(PageInfo page, int firstToDelete)
        {
            ...
            var done = page.Provider.DeleteBackups(page, firstToDelete);
            if (done)
            {
        ...
        public static bool Rollback(PageInfo page, int version)
        {
            ...
            var done = page.Provider.RollbackPage(page, version);
            ...
            if (done)
            {
                ...
                Formatter.Format(pageContent.Content, false, FormattingContext.PageContent, page, out var linkedPages);
                var outgoingLinks = new string[linkedPages.Count];
        ...
        public static bool CreatePage(string nspace, string name, IPagesStorageProviderV30 provider)
        {
            ...
            var fullName = NameTools.GetFullName(nspace, name);
        ...
        public static bool DeletePage(PageInfo page)
        {
            ...
            var title = Content.GetPageContent(page, false).Title;

            var done = page.Provider.RemovePage(page);
            ...
            if (done)
            {
                ...
                foreach (IFilesStorageProviderV30 prov in Collectors.FilesProviderCollector.AllProviders)
                {
                    foreach (var attn in prov.ListPageAttachments(page))
                    {
                ...
                // Do not rebuild all pages links anymore
        ...
        public static bool RenamePage(PageInfo page, string name)
        {
            ...
            var newFullName = NameTools.GetFullName(NameTools.GetNamespace(page.FullName), NameTools.GetLocalName(name));

            if (FindPage(newFullName) != null) return false;

            var oldName = page.FullName;
            ...
            IList<string> keywords, string description, SaveMode saveMode)
        {
            ...
            var done = page.Provider.ModifyPage(page, title, username, dateTime, comment, content, keywords, description, saveMode);
        ...
        {
            Formatter.Format(content, false, FormattingContext.PageContent, page, out var linkedPages);

            var lowercaseName = page.FullName.ToLowerInvariant();

            // Avoid self-references
            var cleanLinkedPages = new List<string>(linkedPages);
            for (var i = cleanLinkedPages.Count - 1; i >= 0; i--)
            {
            ...
            var doneLinks = Settings.Provider.StoreOutgoingLinks(page.FullName, cleanLinkedPages.ToArray());
            if (!doneLinks)
            {
        ...
        {
            var maxBackups = Settings.KeptBackupNumber;
        ...
        private static UserInfo[] RemoveUserFromArray(UserInfo[] users, UserInfo userToRemove)
        {
            ...
            var temp = new List<UserInfo>(users);
        ...
        private static void SendEmailNotificationForPage(PageInfo page, UserInfo author)
        {
            ...
            var recipients = EmailTools.GetRecipients(usersToNotify);

            var body = Settings.Provider.GetMetaDataItem(MetaDataItem.PageChangeMessage, null);

            var title = FormattingPipeline.PrepareTitle(content.Title, false, FormattingContext.Other, page);
        ...
        public static void CanEditPage(PageInfo page, string username, IEnumerable<string> groups,
        ...
        public static bool CanApproveDraft(PageInfo page, string username, IEnumerable<string> groups)
        {
            var requiredAction = Actions.ForPages.ManagePage;
        ...
        public static void SendEmailNotificationForDraft(PageInfo currentPage, string title, string comment, string author)
        {
            ...
            var usersToNotify = new List<UserInfo>(10);
            ...
            var displayName = actualUser == null ? author : Users.GetDisplayName(actualUser);

            var subject = Settings.WikiTitle + " - " + Exchanger.ResourceExchanger.GetResource("ApproveRejectDraft") + ": " + title;
            var body = Settings.Provider.GetMetaDataItem(MetaDataItem.ApproveDraftMessage, null);
        ...
        {
            var allPages = new List<PageInfo>(10000);
        ...
        public static int GetGlobalPageCount()
        {
            ...
            foreach (IPagesStorageProviderV30 prov in Collectors.PagesProviderCollector.AllProviders)
            {
                count += prov.GetPages(null).Count;
                foreach (NamespaceInfo nspace in prov.GetNamespaces())
                {
                    count += prov.GetPages(nspace).Count;
                }
            }
        ...
        public static List<string> GetOrphanedPages(IList<PageInfo> pages)
        {
            ...
            {
                if (GetPageIncomingLinks(p).Length == 0)
                {
        ...
        public static PageInfo[] GetOrphanedPages(NamespaceInfo nspace)
        {
            ...
            var knownPages = new string[allLinks.Count];
            allLinks.Keys.CopyTo(knownPages, 0);

            var result = new Dictionary<PageInfo, bool>(pages.Count);

            foreach (PageInfo page in pages)
            {
                result.Add(page, false);
                foreach (var key in knownPages)
                {
        ...
        public static Dictionary<string, List<string>> GetWantedPages(string nspace)
        {
            ...
            var result = new Dictionary<string, List<string>>(100);

            foreach (var key in knownPages)
            {
                foreach (var link in allLinks[key])
                {
                    var linkNamespace = NameTools.GetNamespace(link);
        ...
        public static bool RemoveCategory(CategoryInfo category)
        {
            ...
            var done = category.Provider.RemoveCategory(category);
        ...
        public static bool RenameCategory(CategoryInfo category, string newName)
        {
            ...
            var newFullName = NameTools.GetFullName(NameTools.GetNamespace(category.FullName), newName);

            if (FindCategory(newFullName) != null) return false;

            var oldName = category.FullName;
        ...
        public static IList<CategoryInfo> GetCategoriesForPage(PageInfo page)
        {
            if (page == null) return new CategoryInfo[0];

            return page.Provider.GetCategoriesForPage(page);
        }
        ...
        public static IList<PageInfo> GetUncategorizedPages(NamespaceInfo nspace)
        {
            if (nspace == null)
            {
                var pages = new List<PageInfo>(1000);
            ...
            {
                return nspace.Provider.GetUncategorizedPages(nspace);
            }
        }
        ...
        public static IList<CategoryInfo> GetAvailableCategories(PageInfo page)
        {
        ...
        public static bool Rebind(PageInfo page, CategoryInfo[] cats)
        {
            ...
            var done = page.Provider.RebindPage(page, names);
        ...
        {
            var allCategories = new List<CategoryInfo>(50);
        ...
        public static IList<Message> GetPageMessages(PageInfo page)
        {
        ...
        public static Message FindMessage(IEnumerable<Message> messages, int id)
        {
            Message result = null;
            foreach (var message in messages)
            {
                ...
                if (result == null)
                {
                    result = FindMessage(message.Replies, id);
                }
        ...
        public static bool AddMessage(PageInfo page, string username, string subject, DateTime dateTime, string body, int parent)
        {
            ...
            var done = page.Provider.AddMessage(page, username, subject, dateTime, body, parent);
            if (done)
            {
        ...
        private static void SendEmailNotificationForMessage(PageInfo page, UserInfo author, string id, string subject, DateTime dateTime)
        {
            ...
            var body = Settings.Provider.GetMetaDataItem(MetaDataItem.DiscussionChangeMessage, null);
        ...
        public static bool RemoveMessage(PageInfo page, int id, bool removeReplies)
        {
            ...
            var messages = page.Provider.GetMessages(page);
            Message msg = FindMessage(messages, id);

            var done = page.Provider.RemoveMessage(page, id, removeReplies);
            if (done)
            {
                PageContent content = Content.GetPageContent(page, false);
        ...
        public static bool RemoveAllMessages(PageInfo page)
        {
            ...
            var messages = GetPageMessages(page);
        ...
        public static bool ModifyMessage(PageInfo page, int id, string username, string subject, DateTime dateTime, string body)
        {
            ...
            var done = page.Provider.ModifyMessage(page, id, username, subject, dateTime, body);
            if (done)
            {
                PageContent content = Content.GetPageContent(page, false);
