...
namespace ScrewTurn.Wiki
{
    ...
    public class SettingsStorageProvider : ProviderBase, ISettingsStorageProviderV30
    {
        ...
        public IDictionary<string, string> GetAllSettings()
        {
            lock (this)
            {
                var result = new Dictionary<string, string>(configData.Count);
        ...
        private void LoadConfig()
        {
            ...
            lock (this)
            {
                ...
                var data = File.ReadAllText(GetFullPath(ConfigFile), System.Text.UTF8Encoding.UTF8);
        ...
        private void DumpConfig()
        {
            lock (this)
            {
                ...
                var keys = new string[configData.Keys.Count];
        ...
        public void LogEntry(string message, EntryType entryType, string user)
        {
            ...
            lock (this)
            {
                ...
                user = Sanitize(user);
                var level = (LoggingLevel)Enum.Parse(typeof(LoggingLevel), host.GetSettingValue(SettingName.LoggingLevel));
                switch (level)
                {
                ...
                var sw = new StreamWriter(fs, System.Text.UTF8Encoding.UTF8);
                ...
                try
                {
                    var fi = new FileInfo(GetFullPath(LogFile));
                    if (fi.Length > int.Parse(host.GetSettingValue(SettingName.MaxLogSize)) * 1024)
                    {
        ...
        private void CutLog(int size)
        {
            lock (this)
            {
                ...
                var entries = new List<LogEntry>(GetLogEntries());
                entries.Reverse();

                var fi = new FileInfo(GetFullPath(LogFile));
                var difference = (int)(fi.Length - size);
                var removeEntries = difference / EstimatedLogEntrySize * 2; // Double the number of removed entries in order to reduce the # of times Cut is needed
                var preserve = entries.Count - removeEntries; // The number of entries to be preserved

                // Copy the entries to preserve in a temp list
                var toStore = new List<LogEntry>();
        ...
        public LogEntry[] GetLogEntries()
        {
            lock (this)
            {
                var content = File.ReadAllText(GetFullPath(LogFile)).Replace("\r", "");
                var result = new List<LogEntry>(50);
        ...
        {
            var targetFile = (!string.IsNullOrEmpty(tag) ? tag + "." : "") + file;
        ...
        public string GetMetaDataItem(MetaDataItem item, string tag, bool nullTagIfMissing)
        {
            lock (this)
            {
                var fullFile = GetFullPathForMetaDataItem(tag, MetaDataItemFiles[item]);
                if (File.Exists(fullFile))
                {
                    return File.ReadAllText(fullFile);
                }

                if (nullTagIfMissing && !string.IsNullOrEmpty(tag))
                {
                    fullFile = GetFullPathForMetaDataItem(string.Empty, MetaDataItemFiles[item]);
                    if (File.Exists(fullFile))
                    {
                        return File.ReadAllText(fullFile);
                    }
        ...
        public bool SetMetaDataItem(MetaDataItem item, string tag, string content)
        {
            lock (this)
            {
                var path = GetFullPathForMetaDataItem(tag, MetaDataItemFiles[item]);
                if (content == null)
                {
        ...
        public List<RecentChange> GetRecentChanges()
        {
            ...
            {
                // Load from file
                var data = File.ReadAllText(GetFullPath(RecentChangesFile)).Replace("\r", "");
                var lines = data.Split(new char[] { '\n' }, StringSplitOptions.RemoveEmptyEntries);

                var changes = new List<RecentChange>(lines.Length);
            ...
            ScrewTurn.Wiki.PluginFramework.Change change, string descr)
        {
            ...
            lock (this)
            {
                ...
                var max = int.Parse(host.GetSettingValue(SettingName.MaxRecentChanges));
                if (GetRecentChanges().Count > max) CutRecentChanges((int)(max * 0.90));
        ...
        private void CutRecentChanges(int size)
        {
            lock (this)
            {
                var changes = new List<RecentChange>(GetRecentChanges());
                if (size >= changes.Count) return;

                var idx = changes.Count - size + 1;

                var sb = new StringBuilder();

                for (var i = idx; i < changes.Count; i++)
                {
        ...
        public string[] ListPluginAssemblies()
        {
            lock (this)
            {
                var files = Directory.GetFiles(GetFullPath(PluginsDirectory), "*.dll");
                var result = new string[files.Length];
                for (var i = 0; i < files.Length; i++) result[i] = Path.GetFileName(files[i]);
                return result;
            }
        }
        ...
        public bool DeletePluginAssembly(string filename)
        {
            ...
            lock (this)
            {
                var fullName = GetFullPathForPlugin(filename);
        ...
        public bool SetPluginStatus(string typeName, bool enabled)
        {
            ...
            lock (this)
            {
                var data = File.ReadAllText(GetFullPathForPlugin(PluginsStatusFile)).Replace("\r", "");
                var lines = data.Split(new char[] { '\n' }, StringSplitOptions.RemoveEmptyEntries);
                var idx = -1;
                for (var i = 0; i < lines.Length; i++)
                {
        ...
        public string GetPluginConfiguration(string typeName)
        {
            ...
            lock (this)
            {
                var file = GetFullPathForPluginConfig(typeName + ".cs");
        ...
        public bool StoreOutgoingLinks(string page, string[] outgoingLinks)
        {
            ...
            lock (this)
            {
                ...
                var lines = File.ReadAllLines(GetFullPath(LinksFile));
        ...
        /// <summary>
        /// Gets the incoming links of a page.
        /// </summary>
        /// <param name="page">The full name of the page.</param>
        ...
        public bool DeleteOutgoingLinks(string page)
        {
            ...
            lock (this)
            {
                ...
                {
                    var currentLinks = new List<string>(links[key]);
                ...
                if (removedSomething)
                {
                    var sb = new StringBuilder(links.Count * 100);
