...
namespace ScrewTurn.Wiki
{
    ...
    public static class DataMigrator
    {
        ...
        {
            // Move Snippets
            var snippets = source.GetSnippets();
            ...
            // Create namespaces
            var namespaces = source.GetNamespaces();
            var createdNamespaces = new NamespaceInfo[namespaces.Count];
            for (var i = 0; i < namespaces.Count; i++)
            {
                createdNamespaces[i] = destination.AddNamespace(namespaces[i].Name);
            }

            var sourceNamespaces = new List<NamespaceInfo>();
            ...
            foreach (NamespaceInfo currentNamespace in sourceNamespaces)
            {
                ...
                var sourceNavPaths = source.GetNavigationPaths(currentNamespace);

                // Copy categories (removed from source later)
                var sourceCats = source.GetCategories(currentNamespace);
                for (var i = 0; i < sourceCats.Count; i++)
                {
                    destination.AddCategory(NameTools.GetNamespace(sourceCats[i].FullName), NameTools.GetLocalName(sourceCats[i].FullName));
                }

                // Move Pages
                var pages = source.GetPages(currentNamespace);
                for (var i = 0; i < pages.Count; i++)
                {
                    ...
                    var revs = source.GetBackups(pages[i]);
                    ...
                    // Move all messages
                    var messages = source.GetMessages(pages[i]);
                    destination.BulkStoreMessages(newPage, messages);

                    // Bind current page (find all proper categories, and use them to bind the page)
                    var pageCats = new List<string>();
                    for (var k = 0; k < sourceCats.Count; k++)
                    {
                        for (var z = 0; z < sourceCats[k].Pages.Count; z++)
                        {
                ...
                var newPages = new List<PageInfo>(destination.GetPages(currentNamespace == null ? null : createdNamespaces[currentNamespaceIndex]));
                for (var i = 0; i < sourceNavPaths.Count; i++)
                {
                    var tmp = new PageInfo[sourceNavPaths[i].Pages.Length];
        ...
        public static void MigrateFilesStorageProviderData(IFilesStorageProviderV30 source, IFilesStorageProviderV30 destination, ISettingsStorageProviderV30 settingsProvider)
        {
            ...
            // Attachments
            foreach (var page in source.GetPagesWithAttachments())
            {
                var pageInfo = new PageInfo(page, null, DateTime.Now);

                var attachments = source.ListPageAttachments(pageInfo);
        ...
        public static void CopySettingsStorageProviderData(ISettingsStorageProviderV30 source, ISettingsStorageProviderV30 destination,
            IEnumerable<string> knownNamespaces, IEnumerable<string> knownPlugins)
        {
            ...
            // Plugin assemblies
            var assemblies = source.ListPluginAssemblies();
            ...
            foreach (var plug in knownPlugins)
            {
            ...
            // ACLs
            var allEntries = source.AclManager.RetrieveAllEntries();
