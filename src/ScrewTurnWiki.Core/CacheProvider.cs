...
namespace ScrewTurn.Wiki
{
    ...
    public class CacheProvider : ICacheProviderV30
    {
        ...
        // for each page, contains page title.
        private readonly ConcurrentDictionary<PageInfo, string> _pageTitleCache = new ConcurrentDictionary<PageInfo, string>();
        ...
        private Dictionary<string, PageRedirection> _redirections;
        ...
        public void Init(IHostV30 host, string config)
        {
            ...
            var s = int.Parse(host.GetSettingValue(SettingName.CacheSize));
            ...
            _redirections = new Dictionary<string, PageRedirection>(50);
        }
        ...
        public void SetPageContent(PageInfo pageInfo, PageContent content)
        {
            ...
            lock (_pageContentCache)
            {
                ...
                _pageTitleCache.AddOrUpdate(pageInfo, content.Title, (_, __) => content.Title);
        ...
        /// <summary>
        ///     Gets the Title of a Page, previously stored in cache.
        /// </summary>
        /// <param name="pageInfo">The Page Info object related to the title being requested.</param>
        /// <returns>The page title, or <c>null</c> if the item is not found.</returns>
        ...
        {
            if (pageInfo == null) throw new ArgumentNullException(nameof(pageInfo));
            if (_pageTitleCache.TryGetValue(pageInfo, out var title))
                return title;
            return null;
        }
        ...
        /// <exception cref="ArgumentNullException">If <b>pageInfo</b> or <b>content</b> are <c>null</c>.</exception>
        public void SetPageTitle(PageInfo pageInfo, string title)
        {
            if (pageInfo == null) throw new ArgumentNullException(nameof(pageInfo));

            lock (_pageContentCache)
            {
                _pageTitleCache.AddOrUpdate(pageInfo, title, (_, __) => title);
            }
        }
        ...
        public void RemovePage(PageInfo pageInfo)
        {
            ...
            lock (_pageContentCache)
            {
                ...
                _pageTitleCache.TryRemove(pageInfo, out _);
            }
        ...
        {
            if (cutSize <= 0) throw new ArgumentOutOfRangeException(nameof(cutSize), "Cut Size should be greater than zero");
        ...
        public void RenewEditingSession(string page, string user)
        {
            ...
            if (page.Length == 0) throw new ArgumentException("Page cannot be empty", nameof(page));
            if (user == null) throw new ArgumentNullException(nameof(user));
            if (user.Length == 0) throw new ArgumentException("User cannot be empty", nameof(user));
        ...
        public bool IsPageBeingEdited(string page, string currentUser)
        {
            ...
            if (currentUser.Length == 0) throw new ArgumentException("Current User cannot be empty", nameof(currentUser));

            lock (this)
            {
                var timeout = int.Parse(_host.GetSettingValue(SettingName.EditingSessionTimeout));
        ...
        public void RemovePageFromRedirections(string name)
        {
            ...
            lock (_redirections)
            {
                ...
                var keysToRemove = new List<string>(10);

                foreach (var pair in _redirections)
                {
                    if (pair.Value.FullName.ToLowerInvariant() == name)
                    {
