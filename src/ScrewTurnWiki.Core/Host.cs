...
namespace ScrewTurn.Wiki
{
    ...
    public class Host : IHostV30
    {
        ...
        public string GetSettingValue(SettingName name)
        {
            switch (name)
            {
                ...
                case SettingName.AllowedFileTypes:
                    ...
                    foreach (var s in Settings.AllowedFileTypes) sb.Append(s + ",");
        ...
        public IList<CategoryInfo> GetCategoriesPerPage(PageInfo page)
        {
            if (page == null) throw new ArgumentNullException("page");
        ...
        public string PrepareContentForIndexing(PageInfo page, string content)
        {
            ...
            return Tools.RemoveHtmlMarkup(FormattingPipeline.FormatWithPhase1And2(content, true, page != null ? FormattingContext.PageContent : FormattingContext.Unknown, page));
        }
        ...
        {
            var result = new List<StDirectoryInfo>(20);
            ...
            {
                var dirs = directory.Provider.ListDirectories(directory.FullPath);
        ...
        {
            var result = new List<StFileInfo>(20);
            ...
            {
                var files = directory.Provider.ListFiles(directory.FullPath);
        ...
        public StFileInfo[] ListPageAttachments(PageInfo page)
        {
            ...
            var result = new List<StFileInfo>(10);
            foreach (IFilesStorageProviderV30 prov in Collectors.FilesProviderCollector.AllProviders)
            {
                var attachments = prov.ListPageAttachments(page);

                foreach (var attn in attachments)
                {
        ...
        {
            var timezone = Preferences.LoadTimezoneFromCookie() ?? Settings.DefaultTimezone;
        ...
        public IEnumerable<IPagesStorageProviderV30> GetPagesStorageProviders(bool enabled)
        {
        ...
        /// <param name="enabled"><c>true</c> to get enabled providers, <c>false</c> to get disabled providers.</param>
        /// <returns>The providers.</returns>
        public IEnumerable<IUsersStorageProviderV30> GetUsersStorageProviders(bool enabled)
        {
        ...
        /// <param name="enabled"><c>true</c> to get enabled providers, <c>false</c> to get disabled providers.</param>
        /// <returns>The providers.</returns>
        public IEnumerable<IFilesStorageProviderV30> GetFilesStorageProviders(bool enabled)
        {
        ...
        /// <param name="enabled"><c>true</c> to get enabled providers, <c>false</c> to get disabled providers.</param>
        /// <returns>The providers.</returns>
        public IEnumerable<ICacheProviderV30> GetCacheProviders(bool enabled)
        {
        ...
        /// <param name="enabled"><c>true</c> to get enabled providers, <c>false</c> to get disabled providers.</param>
        /// <returns>The providers.</returns>
        public IEnumerable<IFormatterProviderV30> GetFormatterProviders(bool enabled)
        {
