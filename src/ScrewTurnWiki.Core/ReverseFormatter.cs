...
namespace ScrewTurn.Wiki
{
    ...
    public static class ReverseFormatter
    {
        ...
        private static string ProcessLink(string link)
        {
            ...
            {
                var urlParameters = link.Remove(0, 13).Split(new char[] { '&' }, StringSplitOptions.RemoveEmptyEntries);

                var pageName = urlParameters.FirstOrDefault(p => p.ToLowerInvariant().StartsWith("page"));
                if (!string.IsNullOrEmpty(pageName)) pageName = Uri.UnescapeDataString(pageName.Split(new char[] { '=' })[1]);
                var fileName = urlParameters.FirstOrDefault(p => p.ToLowerInvariant().StartsWith("file"));
        ...
        public static string ReverseFormat(string html)
        {
            ...
            XmlDocument x = FromHTML(strReader);
