...
namespace ScrewTurn.Wiki
{
    ...
    public static class Formatter
    {
        ...
        private static readonly Regex LinkRegex = new Regex(@"(\[\[.{0,200}?\]\])|(\[.{0,200}?\])", RegexOptions.Compiled);
        private static readonly Regex DoubleSquareLinkRegex = new Regex(@"(\[\[.{0,200}?\]\])", RegexOptions.Compiled);
        ...
        private static readonly Regex BaseRegex = new Regex(@"\{basedir\}", RegexOptions.IgnoreCase);
        ...
        {
            get { return Settings.IgnoreSingleSquareBrackets ? DoubleSquareLinkRegex : LinkRegex; }
        }
        ...
        private static NamespaceInfo DetectNamespaceInfo(PageInfo currentPage)
        {
            ...
            {
                var ns = NameTools.GetNamespace(currentPage.FullName);
        ...
        /// <returns>The formatted text.</returns>
        public static string Format(string raw, bool forIndexing, FormattingContext context, PageInfo current, out IList<string> linkedPages)
        {
        ...
        /// <returns>The formatted text.</returns>
        public static string Format(string raw, bool forIndexing, FormattingContext context, PageInfo current, out IList<string> linkedPages, bool bareBones)
        {
            ...
            if (current != null)
            {
                ...
                if (match.Success)
                {
                    if (!IsNoWikied(match.Index, noWikiBegin, noWikiEnd, out end))
                    {
                        ...
                        if (i != -1)
                        {
                            fragment = destination.Substring(i + 1);
                        ...
                        if (dest != null)
                        {
                            Redirections.AddRedirection(current, new PageRedirection { FullName = destination, Fragment = fragment });
                        }
                    }
                    ComputeNoWiki(sb.ToString(), ref noWikiBegin, ref noWikiEnd);
                }
                        ...
                        if (!forIndexing)
                        {
                            switch (match.Value.Substring(1, match.Value.Length - 2).ToUpperInvariant())
                            {
                                ...
                                case "TOP":
                                    ...
                                    break;
                                case "SEARCHBOX":
                                    var textBoxId = "SB" + Guid.NewGuid().ToString("N");

                                    var nsstring = ns != null ? NameTools.GetFullName(ns.Name, "Search") + ".aspx" : "Search.aspx";
                                    var doSearchFunction = "<nowiki><nobr><script type=\"text/javascript\"><!--\r\n" + @"function _DoSearch_" + textBoxId + "() { document.location = '" + nsstring + @"?AllNamespaces=1&FilesAndAttachments=0&Mode=3&Query=' + encodeURI(document.getElementById('" + textBoxId + "').value); }" + "\r\n// -->\r\n</script>";
                                ...
                                case "CATEGORIES":
                                    ...
                                    var pageName = ns != null ? NameTools.GetFullName(ns.Name, "AllPages") + ".aspx" : "AllPages.aspx";
                                    pageName += "?Cat=";
                                    var categories = "<ul><li>" + string.Join("</li><li>",
                                        (from c in cats
                                    ...
                                    break;
                                case "CLOUD":
                                    var cloud = BuildCloud(DetectNamespaceInfo(current));
                ...
                while (match.Success)
                {
                    if (!IsNoWikied(match.Index, noWikiBegin, noWikiEnd, out end))
                    {
                        ...
                        else
                        {
                            ...
                            var lastOpen = 0;
                            var firstClosedAfterLastOpen = 0;
        ...
        private static void EncodeFilename(StringBuilder buffer, int startIndex)
        {
            ...
            if (endIndex > startIndex)
            {
                ...
                var input = Tools.UrlDecode(allData.Substring(startIndex, len));
                var value = Tools.UrlEncode(input).Replace("%7b", "{").Replace("%7B", "{").Replace("%7d", "}").Replace("%7D", "}").Replace("%3a", ":").Replace("%3A", ":");
        ...
        {
            var allChanges = new List<RecentChange>(RecentChanges.GetAllChanges());
            ...
            var currentNamespaceName = getName(currentNamespace);
        ...
        {
            var maxChanges = Math.Min(Settings.MaxRecentChangesToDisplay, allChanges.Count);
        ...
        private static string BuildWantedPagesList(NamespaceInfo nspace)
        {
            ...
            foreach (var page in wanted.Keys)
            {
                sb.Append("<li>");
        ...
        private static string BuildIncomingLinksList(PageInfo page, FormattingContext context, PageInfo current)
        {
            ...
            var links = Pages.GetPageIncomingLinks(page);
        ...
        private static string BuildOutgoingLinksList(PageInfo page, FormattingContext context, PageInfo current)
        {
            ...
            var links = Pages.GetPageOutgoingLinks(page);
            if (links.Length == 0) return "";
        ...
        private static string FormatSnippet(string capturedMarkup, string cachedToc)
        {
            ...
            var tempResult = Snippets.ParametersRegex.Replace(output.ToString(), "");
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (forIndexing)
                            {
                                ...
                                {
                                    // Try to format title
                                    var titleText = Content.GetPageTitle(info, false);
                                    if (!string.IsNullOrEmpty(titleText))
                                    {
                                        sb.Append(FormattingPipeline.PrepareTitle(titleText, forIndexing, context, currentPage));
                                    }
                            ...
                            {
                                var titleText = Content.GetPageTitle(info, false);
                                titleText = FormattingPipeline.PrepareTitle(titleText, forIndexing, context, currentPage);
                                sb.Append(titleText);
                            }
        ...
        private static string BuildCloud(NamespaceInfo currentNamespace)
        {
            ...
            var tot = Pages.GetPages(currentNamespace).Count - Pages.GetUncategorizedPages(currentNamespace).Count;
            List<CategoryInfo> categories = Pages.GetCategories(currentNamespace);
            for (var i = 0; i < categories.Count; i++)
            {
                if (categories[i].Pages.Count > 0)
                {
                    //sb.Append(@"<a href=""AllPages.aspx?Cat=");
                    ...
                    sb.Append(ComputeSize(categories[i].Pages.Count / (float)tot * 100F).ToString());
                    sb.Append(@"px;"">");
        ...
        public static string FormatPhase3(string raw, FormattingContext context, PageInfo current)
        {
            ...
            while (match.Success)
            {
                ...
                {
                    case "NAMESPACE":
                        var ns = Tools.DetectCurrentNamespace();
            ...
            if (current != null)
            {
                ...
                {
                    sb.Remove(match.Index, match.Length);
                    var trimmedTag = match.Value.Trim('{', '}');
        ...
        private static string BuildCurrentNamespaceDropDown()
        {
            ...
            var allowedNamespaces = new List<string>(allNamespaces.Count);
            ...
            foreach (var nspace in allowedNamespaces)
            {
        ...
        {
            var login = Exchanger.ResourceExchanger.GetResource("Login");
            var sb = new StringBuilder(200);
            sb.Append("<a href=\"");
        ...
        {
            var login = Exchanger.ResourceExchanger.GetResource("Logout");
            var sb = new StringBuilder(200);
            sb.Append("<a href=\"");
