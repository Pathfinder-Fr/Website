...
namespace ScrewTurn.Wiki.AclEngine
{
    ...
    public static class AclEvaluator
    {
        ...
        ///     If <paramref name="resource" />, <paramref name="action" />,
        ///     <paramref name="user" />, <paramref name="groups" /> or <paramref name="entries" /> are <c>null</c>.
        /// </exception>
        /// <exception cref="ArgumentException">
        ///     If <paramref name="resource" />, <paramref name="action" />,
        ///     <paramref name="user" /> are empty, or if <paramref name="action" /> equals
        ///     <see cref="AclEntry.FullControlAction" />.
        /// </exception>
        public static Authorization AuthorizeAction(string resource, string action, string user, IEnumerable<string> groups, IEnumerable<AclEntry> entries)
        {
            ...
            //var sortedEntries = new AclEntry[entries.Length];
            //Array.Copy(entries, sortedEntries, entries.Length);

            //Array.Sort(sortedEntries, delegate(AclEntry x, AclEntry y) { return x.Action.CompareTo(y.Action); });
            //Array.Reverse(sortedEntries);

            foreach (var entry in entries.OrderByDescending(x => x.Action))
            {
                if (entry.Resource == resource
                    && (entry.Action == action || entry.Action == AclEntry.FullControlAction)
                    && entry.Subject == user)
                {
            ...
            var groupFullControlGrant = new Dictionary<string, bool>();
            var groupExplicitGrant = new Dictionary<string, bool>();
            var groupFullControlDeny = new Dictionary<string, bool>();
            ...
            var tentativeGrant = false;
            var tentativeDeny = false;
            foreach (var group in groupFullControlGrant.Keys)
            {
