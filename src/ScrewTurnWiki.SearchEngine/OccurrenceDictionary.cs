...
namespace ScrewTurn.Wiki.SearchEngine
{
    ...
    {
        private readonly Dictionary<IDocument, SortedBasicWordInfoSet> dictionary;
        private readonly Dictionary<string, IDocument> mappings; // Used for performance purposes
        ...
        public List<DumpedWordMapping> RemoveExtended(IDocument key, uint wordId)
        {
            ...
            var mappedDoc = mappings[key.Name];
            if (mappings.Remove(target.Name))
            {
                // Prepare the list of DumpedWordMapping objects
                var set = dictionary[mappedDoc];
                var dump = new List<DumpedWordMapping>(set.Count);
