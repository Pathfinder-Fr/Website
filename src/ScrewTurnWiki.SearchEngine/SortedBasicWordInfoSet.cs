...
namespace ScrewTurn.Wiki.SearchEngine
{
    ...
    {
        private readonly List<BasicWordInfo> items;
            ...
            get
            {
                if (index < 0 || index > items.Count - 1)
                    throw new IndexOutOfRangeException(
                        "Index should be greater than or equal to zero and less than the number of items in the set");
                return items[index];
            }
        }
        ...
        /// <remarks>
        ///     Adding an item is <b>O(log n)</b> if the item is already in the set,
        ///     <b>O(n)</b> otherwise, where <b>n</b> is the number of items in the set.
        /// </remarks>
        public bool Add(BasicWordInfo item)
        {
            var idx = items.BinarySearch(item);
