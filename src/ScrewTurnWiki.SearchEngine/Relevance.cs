...
namespace ScrewTurn.Wiki.SearchEngine
{
    ...
    public class Relevance
    {
        ...
        ///     If <see cref="M:IsFinalized" /> is <c>true</c> (<see cref="M:Finalize" />
        ///     was called).
        /// </exception>
        public void SetValue(float value)
        {
            if (value < 0)
                throw new ArgumentOutOfRangeException("value", "Value must be greater than or equal to zero");
            if (isFinalized)
                throw new InvalidOperationException("After finalization, the value cannot be changed anymore");
            this.value = value;
        }
        ...
        /// <exception cref="InvalidOperationException">
        ///     If <see cref="IsFinalized" /> is <c>true</c> (<see cref="M:Finalize" /> was
        ///     called).
        /// </exception>
        public void Finalize(float total)
        {
            if (total < 0)
                throw new ArgumentOutOfRangeException("total", "Total must be greater than or equal to zero");
        ...
        /// <exception cref="InvalidOperationException">
        ///     If <see cref="IsFinalized" /> is <c>false</c> (<see cref="M:Finalize" />
        ///     was not called).
        /// </exception>
