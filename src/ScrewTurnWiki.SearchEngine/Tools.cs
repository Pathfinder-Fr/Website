...
namespace ScrewTurn.Wiki.SearchEngine
{
    ...
    public static class Tools
    {
        ...
        public static IList<string> CleanupKeywords(IList<string> keywords)
        {
            ...
            {
                var temp = RemoveDiacriticsAndPunctuation(k.Replace(" ", ""), true);
                if (temp.Length > 0)
                {
                    result.Add(temp);
                }
        ...
        {
            var cat = char.GetUnicodeCategory(current);
        ...
        public static WordInfo[] Tokenize(string text, WordLocation location)
        {
            ...
            var words = new List<WordInfo>(text.Length/5); // Average 5 chars/word
            ...
            {
                while (currentIndex < text.Length && !IsSplitChar(text[currentIndex])) currentIndex++;
                var w = text.Substring(currentWordStart, currentIndex - currentWordStart);
                w = RemoveDiacriticsAndPunctuation(w, true);
        ...
        public static WordInfo[] RemoveStopWords(WordInfo[] words, string[] stopWords)
        {
            ...
            var result = new List<WordInfo>(words.Length);
        ...
        ///     If <paramref name="filterDocumentType" /> is <c>true</c> and
        ///     <paramref name="documentTypeTags" /> is <c>null</c>.
        /// </exception>
        /// <exception cref="ArgumentException">
        ///     If <paramref name="filterDocumentType" /> is <c>true</c> and
        ///     <paramref name="documentTypeTags" /> is empty.
        /// </exception>
        public static SearchResultCollection SearchInternal(string query, string[] documentTypeTags,
            bool filterDocumentType, SearchOptions options, IWordFetcher fetcher)
        {
        ...
        public static float PurgeResultsForAllWords(SearchResultCollection results, string[] queryWords)
        {
            ...
            var toRemove = new List<SearchResult>();
        ...
        public static float PurgeResultsForExactPhrase(SearchResultCollection results, string[] queryWords)
        {
            ...
            foreach (var r in results)
            {
                ...
                else
                {
                    ...
                    for (var shift = 0; shift <= maxTestShift; shift++)
                    {
