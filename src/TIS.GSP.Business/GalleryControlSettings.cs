	...
	public class GalleryControlSettings : IGalleryControlSettings
	{
		...
		internal static IGalleryControlSettingsCollection RetrieveGalleryControlSettingsFromDataStore()
		{
			...
			try
			{
				...
				using (dr = Factory.GetDataProvider().GalleryControlSetting_GetDataReaderGalleryControlSettings())
				{
					...
					while (dr.Read())
					{
						...
						string currControlId = dr["ControlId"].ToString().Trim();
						...
						if (String.IsNullOrEmpty(prevControlId) || (!currControlId.Equals(prevControlId)))
						{
							...
							int id = Convert.ToInt32(dr["GalleryControlSettingId"].ToString().Trim(), CultureInfo.InvariantCulture);

							gs = new GalleryControlSettings(id, currControlId);
						...
						if (prop == null)
						{
							throw new MissingMemberException(string.Format("Invalid gallery control setting. A gallery control setting named '{0}' was found in the data store, but no property by that name exists in the class '{1}'. Check the gallery control settings in the data store to ensure they are correct.", settingName, gsType));
						}
						...
						{
							AssignViewModeProperty(gs, prop, dr["SettingValue"].ToString().Trim());
						}
						else if (prop.PropertyType == typeof(String[]))
						{
						...
						{
							throw new ArgumentOutOfRangeException(String.Format("GalleryControlSettings.RetrieveGalleryControlSettingsFromDataStore is not designed to process a property of type {0} (encountered in GalleryControlSettings.{1})", prop.PropertyType, prop.Name));
						}
		...
		private static void AssignViewModeProperty(IGalleryControlSettings galleryControlSetting, PropertyInfo property, string value)
		{
			...
			{
				throw new ArgumentOutOfRangeException(String.Format("GallerySetting.RetrieveGalleryControlSettingsFromDataStore cannot convert the string {0} to a ViewMode enumeration value. The following values are valid: NotSet, Multiple, Single, SingleRandom", value), ex);
			}
