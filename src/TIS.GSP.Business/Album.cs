...
namespace GalleryServerPro.Business
{
	...
	public class Album : GalleryObject, IAlbum
	{
		...
		internal Album(int id, int galleryId, int parentId, string title, string directoryName, string summary, int thumbnailMediaObjectId, int sequence, DateTime dateStart, DateTime dateEnd, string createdByUsername, DateTime dateAdded, string lastModifiedByUsername, DateTime dateLastModified, string ownerUsername, string ownerRoleName, bool isPrivate)
		{
			...
			{
				throw new ArgumentOutOfRangeException("galleryId", string.Format("Gallery ID must be set to a valid value. Instead, the value was {0}.", galleryId));
			}
		...
		public override string FullPhysicalPathOnDisk
		{
			get
			{
				...
				throw new BusinessException(string.Format("Invalid object state. Album.IsNew = {0}, Album.IsInflated = {1}, and the private member variable _fullPhysicalPathOnDisk is either null or empty.", this.IsNew, this.IsInflated));
			}
			set
			{
		...
		//TODO: Consider reworking the default position of a new album. Sorted by title? Placed after the last album
		// instead of the last media object? (but need to deal with a user rearranging albums throughout the media objects.)
		...
		public override IGalleryObject CopyTo(IAlbum destinationAlbum, string userName)
		{
			...
			IAlbum albumCopy = Factory.CreateEmptyAlbumInstance(destinationAlbum.GalleryId);
		...
		{
			const int maxLength = DataConstants.AlbumSummaryLength;
		...
		private void ValidateDirectoryName()
		{
			...
			if (this.DirectoryName.Length > maxLength)
				throw new BusinessException(String.Format("Invalid directory name. The maximum length for a directory name is {0} characters, but one was specified that is {1} characters. More info: album ID = {2}; album title = '{3}'", maxLength, this.DirectoryName.Length, this.Id, this.Title));
		}
		...
		private IDisplayObject GetDefaultAlbumThumbnail()
		{
			...
			IDisplayObject albumThumbnail = DisplayObject.CreateInstance(new NullObjects.NullGalleryObject(), defaultFilename, width, height, DisplayObjectType.Thumbnail, new NullObjects.NullDisplayObjectCreator());
