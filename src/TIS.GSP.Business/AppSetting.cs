	...
	public class AppSetting : IAppSetting
	{
		...
		public int MaxNumberErrorItems
		{
			...
			set
			{
				if (value < 0)
				{
					throw new ArgumentOutOfRangeException("value", string.Format("Invalid MaxNumberErrorItems setting: The value must be between 0 and {0}. Instead, the value was {1}.", Int32.MaxValue, value));
				}
		...
		private void PopulateAppSettingsFromDataStore()
		{
			...
			using (IDataReader dr = Factory.GetDataProvider().AppSetting_GetDataReaderAppSettings())
			{
				while (dr.Read())
				{
					string settingName = dr["SettingName"].ToString().Trim();

					PropertyInfo prop = asType.GetProperty(settingName);

					if (prop == null)
					{
						throw new MissingMemberException(string.Format("Invalid application setting. An application setting named '{0}' was found in the data store, but no property by that name exists in the class '{1}'. Check the application settings in the data store to ensure they are correct.", settingName, asType));
					}
					else if (prop.PropertyType == typeof(bool))
					{
						prop.SetValue(this, Convert.ToBoolean(dr["SettingValue"].ToString().Trim(), CultureInfo.InvariantCulture), null);
					}
					else if (prop.PropertyType == typeof(string))
					{
						prop.SetValue(this, Convert.ToString(dr["SettingValue"].ToString().Trim(), CultureInfo.InvariantCulture), null);
					}
					else if (prop.PropertyType == typeof(int))
					{
						prop.SetValue(this, Convert.ToInt32(dr["SettingValue"].ToString().Trim(), CultureInfo.InvariantCulture), null);
					}
					else
					{
						throw new ArgumentOutOfRangeException(String.Format("AppSetting.PopulateAppSettingsFromDataStore is not designed to process a property of type {0} (encountered in AppSetting.{1})", prop.PropertyType, prop.Name));
					}
				}
			}
		}
		...
		private void ValidateAppSettings()
		{
			...
			//{
			//  BusinessException ex = new BusinessException(String.Format("Invalid jQuery path. The jQuery path \"{0}\" does not correspond to an existing script file. Any functionality that requires jQuery will not work. Update the jQuery path in the Site admin area.", JQueryScriptPath));
			//  ErrorHandler.Error.Record(ex);
			//}
			...
			//{
			//  BusinessException ex = new BusinessException(String.Format("Invalid jQuery UI path. The jQuery UI path \"{0}\" does not correspond to an existing script file. Any functionality that requires jQuery UI will not work. Update the jQuery UI path in the Site admin area.", JQueryUiScriptPath));
			//  ErrorHandler.Error.Record(ex);
			//}
		}
