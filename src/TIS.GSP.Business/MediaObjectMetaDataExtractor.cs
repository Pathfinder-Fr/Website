...
namespace GalleryServerPro.Business.Metadata
{
	...
	public class MediaObjectMetadataExtractor
	{
				...
				try
				{
					using (System.Drawing.Image image = System.Drawing.Image.FromStream(stream, true, false))
					{
						...
						catch (NotImplementedException ex)
						{
							...
							{
								ex.Data.Add("Metadata Extraction Error", string.Format("Cannot extract metadata from file \"{0}\".", imageFilePath));
							}
		...
		//      string msg = String.Format("{0}: {1} (ID {2}, {3}, {4})", metadataItemName, rawValue, rawMdi.PropertyItem.Id, rawMdi.ExtractedValueType, rawMdi.PropertyTagType);
		...
		private void AddFileMetadata(IGalleryObjectMetadataItemCollection metadataItems)
		{
			...
			metadataItems.Add(new GalleryObjectMetadataItem(int.MinValue, FormattedMetadataItemName.FileSizeKb, Resources.Metadata_FileSize, String.Concat(fileSize.ToString("N0"), " ", Resources.Metadata_KB), true));
