...
namespace GalleryServerPro.Business
{
	...
	public class GalleryObjectCollection : Collection<IGalleryObject>, IGalleryObjectCollection
	{
		...
		public void Sort()
		{
            ...
            // Sueetie Modified - sort by DateAdded for Recent Photos display only
            if (galleryObjects.Count > 0)
            {
                if (DataHelper.GetIntFromQueryString("aid", 1) == -1)
                {
                    galleryObjects.Sort(delegate(IGalleryObject x, IGalleryObject y) { return DateTime.Compare(y.DateAdded, x.DateAdded); });
                }
                else
                    galleryObjects.Sort();
            }
		...
		public new void Add(IGalleryObject galleryObject)
		{
			if (galleryObject == null)
				throw new ArgumentNullException("galleryObject", "Cannot add null to an existing GalleryObjectCollection. Items.Count = " + Items.Count);

			base.Add(galleryObject);
		}
