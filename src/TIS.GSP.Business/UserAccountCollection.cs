...
namespace GalleryServerPro.Business
{
	...
	public class UserAccountCollection : Collection<IUserAccount>, IUserAccountCollection
	{
		...
		/// </overloads>
		/// <summary>
		/// Determines whether the <paramref name="userAccount"/> is a member of the collection. An object is considered a member
		/// of the collection if they both have the same <see cref="IUserAccount.UserName"/>.
		/// </summary>
		/// <param name="userAccount">An <see cref="IUserAccount"/> to determine whether it is a member of the current collection.</param>
		/// <returns>
		/// Returns <c>true</c> if <paramref name="userAccount"/> is a member of the current collection;
		/// otherwise returns <c>false</c>.
		/// </returns>
		public new bool Contains(IUserAccount userAccount)
		{
			foreach (IUserAccount userAccountInCollection in (List<IUserAccount>)Items)
			{
				if (userAccountInCollection.UserName.Equals(userAccount.UserName, StringComparison.Ordinal))
				{
					return true;
				}
			}
		...
		public new void Add(IUserAccount userAccount)
		{
			if (userAccount == null)
				throw new ArgumentNullException("userAccount", "Cannot add null to an existing UserAccountCollection. Items.Count = " + Items.Count);
