...
namespace GalleryServerPro.Business
{
	...
	public class DisplayObject : IDisplayObject
	{
		...
		/// <see cref="DisplayObject"/> is reclaimed by garbage collection.
		/// </summary>
		~DisplayObject()
		{
			Dispose(false);
		...
		/// <exception cref="GalleryServerPro.ErrorHandler.CustomExceptions.InvalidMediaObjectException">Thrown if the file 
			...
			set
			{
				...
				if (value != null)
				{
					...
					if (parentAlbum != null)
					{
						...
						{
							throw new GalleryServerPro.ErrorHandler.CustomExceptions.InvalidMediaObjectException(string.Format(CultureInfo.CurrentCulture, Resources.DisplayObject_FileInfo_Ex_Msg, value.Name, parentAlbum.Id, albumOriginalPath, albumOptimizedPath, albumThumbnailPath));
						}
					}
				}
				...
				this._mimeType = Business.MimeType.LoadMimeType(this.Parent.GalleryId, value.Name);
		...
		/// <summary>
		/// Gets the <see cref="Bitmap"/> associated with the file representing this display object. It is instantiated when
		/// it is first accessed. You must call Dispose() when finished with it. Returns null if <see cref="DisplayType"/>
		/// is Thumbnail or Optimized and the file has not been created for this display object (e.g. When an Image object 
		/// is first created, the thumbnail and optimized images are not created until the instance is saved. Thus this 
		/// property will return null before the <see cref="IGalleryObject.Save"/> method is called). If <see cref="FileInfo"/>
		/// does not represent a valid image, a GalleryServerPro.ErrorHandler.CustomExceptions.UnsupportedImageTypeException is thrown.
		/// </summary>
		/// <value>The bitmap associated with the file representing this display object.</value>
		/// <exception cref="GalleryServerPro.ErrorHandler.CustomExceptions.UnsupportedImageTypeException">Thrown when 
		/// Gallery Server Pro cannot load the <see cref="FileInfo"/> associated with this object into the <see cref="Bitmap"/> class.</exception>
		public System.Drawing.Bitmap Bitmap
		{
			get
			{
				if ((this._bitmap == null) && (this.FileInfo != null))
				{
					try
					{
						try
						{
							this._bitmap = new System.Drawing.Bitmap(this.FileInfo.FullName);
						}
						...
						// Verify that we can access the width and height. Some very large images instantiate correctly, but then throw
						// an ArgumentException (message = "Parameter is not valid") when accessing the width and height.
						// Note: It is uncertain whether this test is doing anything useful. Despite this check, I have observed an
						// ArgumentException being thrown by subsequent code that accesses the width and height.
						try
						{
						...
						{
							throw new UnsupportedImageTypeException(this.Parent, ex);
						}
					}
					catch (ArgumentException ex)
					{
		...
		public string FileName
		{
			...
			set
			{
				...
				if (!String.IsNullOrEmpty(value))
					this._mimeType = Business.MimeType.LoadMimeType(this.Parent.GalleryId, this._filename);
			}
		}
		...
		/// <summary>
		/// Gets or sets the HTML that defines an externally stored media object, such as videos hosted at YouTube or Silverlight.live.com. For local
