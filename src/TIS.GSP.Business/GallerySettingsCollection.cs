...
namespace GalleryServerPro.Business
{
	...
	public class GallerySettingsCollection : Collection<IGallerySettings>, IGallerySettingsCollection
	{
		...
		/// <summary>
		/// Determines whether the <paramref name="gallerySettings"/> is already a member of the collection. An object is considered a member
		/// of the collection if they both have the same <see cref="IGallerySettings.GalleryId"/>.
		/// </summary>
		/// <param name="gallerySettings">An <see cref="IGallerySettings"/> to determine whether it is a member of the current collection.</param>
		/// <returns>
		/// Returns <c>true</c> if <paramref name="gallerySettings"/> is a member of the current collection;
		/// otherwise returns <c>false</c>.
		/// </returns>
		public new bool Contains(IGallerySettings gallerySettings)
		{
			foreach (IGallerySettings gallerySettingsInCollection in (List<IGallerySettings>)Items)
			{
				if (gallerySettingsInCollection.GalleryId == gallerySettings.GalleryId)
				{
					return true;
				}
			}
		...
		/// </summary>
		/// <param name="gallerySettings">The gallery to add.</param>
		public new void Add(IGallerySettings gallerySettings)
		{
			if (gallerySettings == null)
				throw new ArgumentNullException("gallerySettings", "Cannot add null to an existing GallerySettingsCollection. Items.Count = " + Items.Count);

			base.Add(gallerySettings);
		}
	}
}
