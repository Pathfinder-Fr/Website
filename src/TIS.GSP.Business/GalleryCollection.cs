...
namespace GalleryServerPro.Business
{
	...
	public class GalleryCollection : Collection<IGallery>, IGalleryCollection
	{
		...
		public new bool Contains(IGallery gallery)
		{
			foreach (IGallery galleryInCollection in (List<IGallery>)Items)
			{
				if (galleryInCollection.GalleryId == gallery.GalleryId)
				{
					return true;
				}
			}
		...
		public new void Add(IGallery gallery)
		{
			if (gallery == null)
				throw new ArgumentNullException("gallery", "Cannot add null to an existing GalleryCollection. Items.Count = " + Items.Count);
