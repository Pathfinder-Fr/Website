...
(function($, undefined) {
    ...
    function _renderDaySegs(segs, rowCnt, view, minLeft, maxLeft, getRow, dayContentLeft, dayContentRight, segmentContainer, bindSegHandlers, modifiedEventId) {
        ...
        for (i = 0; i < segCnt; i++) {
            ...
            var _description;
            _description = event.description != null ? "<div class='fc-event-description'>" + htmlEscape(event.description) + "</div>" : '';

            var _endRepeatDate;
            _endRepeatDate = event.endRepeatDate;

            var _sourceContentID;
            _sourceContentID = event.sourceContentID;
            
            var _moreMessage;
            _moreMessage = event.url != 'na' ? "<div class='fc-event-moreMessage'>" + htmlEscape(event.moreMessage) + "</div>" : '';

            html +=
			"<div class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;left:" + left + "px'>" +
				"<a" + ((event.url != 'na') ? " href='" + htmlEscape(event.url) + "'" : '') + ">" +
					(!event.allDay && seg.isStart ?
						"<span class='fc-event-time'>" +
							htmlEscape(formatDates(event.start, event.end, view.option('timeFormat'), options)) +
						"</span>"
					: '') +
					"<span class='fc-event-title'>" + htmlEscape(event.title) + "</span>" + _description + _moreMessage +
				"</a>" +
