...
namespace GalleryServerPro.Web.Handler
{
	...
	public class gettreeviewxml : IHttpHandler
	{
		...
		private TreeView GenerateTreeview()
		{
			...
			foreach (IAlbum childAlbum in parentAlbum.GetChildGalleryObjects(GalleryObjectType.Album, true, !Util.IsAuthenticated))
			{
				...
				{
					node.NavigateUrl = String.Format("{0}?aid={1}", _navigateUrl, childAlbum.Id);
				...
				if (SecurityActionEnumHelper.IsValidSecurityAction(this._securityAction))
				{
					isUserAuthorized = Util.IsUserAuthorized(_securityAction, RoleController.GetGalleryServerRolesForUser(), childAlbum.Id, childAlbum.GalleryId, childAlbum.IsPrivate);
				}
				...
				{
					node.ContentCallbackUrl = String.Format(CultureInfo.CurrentCulture, "{0}?aid={1}{2}&sc={3}&nurl={4}", handlerPath, childAlbum.Id.ToString(CultureInfo.InvariantCulture), securityActionParm, node.ShowCheckBox, Util.HtmlEncode(_navigateUrl));
				}
		...
		private bool ExtractQueryStringParms(string queryString)
		{
			...
			foreach (string nameValuePair in queryString.Split(new char[] { '&' }))
			{
				...
				switch (nameOrValue[0])
				{
					...
					case "nurl":
						{
							_navigateUrl = Util.HtmlDecode(nameOrValue[1]).Trim();
							break;
						}
