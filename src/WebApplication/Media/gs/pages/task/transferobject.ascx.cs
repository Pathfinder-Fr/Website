	...
	public partial class transferobject : Pages.TaskPage
	{
		...
		private TransferType TransType
		{
			get
			{
				...
				{
					string qsValue = Util.GetQueryStringParameterString("tt");
					if (qsValue == "move")
		...
		private void RedirectAfterSuccessfulTransfer()
		{
			...
			{
				Util.Redirect(PageId.album, "aid={0}&msg={1}", this.DestinationAlbum.Id.ToString(CultureInfo.InvariantCulture), ((int)msg).ToString(CultureInfo.InvariantCulture));
			}
			else
			{
				Util.Redirect(PageId.album, "&msg={0}", ((int)msg).ToString(CultureInfo.InvariantCulture));
			}
		}
		...
		private void TransferObjects()
		{
			...
			for (int i = 0; i < galleryObjectIds.Length; i++)
			{
				...
				if (idType == 'a')
				{
					try
					{
						IAlbum album = Factory.LoadAlbumInstance(id, false, true);
		...
		private void ValidateObjectsCanBeMovedOrCopied(IGalleryObjectCollection objectsToMoveOrCopy)
		{
			...
			{
				throw new GallerySecurityException(String.Format(CultureInfo.CurrentCulture, "User '{0}' does not have permission '{1}' for album ID {2}.", Util.UserName, SecurityActions.AddChildAlbum, this.DestinationAlbum.Id));
			}
			...
			{
				throw new GallerySecurityException(String.Format(CultureInfo.CurrentCulture, "User '{0}' does not have permission '{1}' for album ID {2}.", Util.UserName, SecurityActions.AddMediaObject, this.DestinationAlbum.Id));
			}
		...
		private void ValidateSecurityForAlbumOrMediaObject(IGalleryObject galleryObjectToMoveOrCopy, SecurityActions securityActions)
		{
			if (this.TransType == TransferType.Move)
			{
				...
				{
					throw new GallerySecurityException(String.Format(CultureInfo.CurrentCulture, "User '{0}' does not have permission '{1}' for album ID {2}.", Util.UserName, securityActions, galleryObjectToMoveOrCopy.Id));
				}
			}
		}
