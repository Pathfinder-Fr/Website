	...
	public partial class mediaobjects : Pages.AdminPage
	{
		...
		private bool ValidatePath(wwDataBindingItem item, string fullPathToTest, Label pathDisplayControl, DisplayObjectType displayType)
		{
			...
			if (this.chkPathIsReadOnly.Checked)
			{
				...
				{
					isValid = ValidateReadOnlyGallery(item) && ValidatePathIsWriteable(item, fullPathToTest);
				}
			}
			else
				isValid = ValidatePathIsWriteable(item, fullPathToTest);
		...
		protected bool wwDataBinder_BeforeUnbindControl(WebControls.wwDataBindingItem item)
		{
			...
			if (!this.chkEnableSlideShow.Checked)
			{
				...
				{
					this.txtSlideShowInterval.Text = GallerySettings.SlideshowInterval.ToString();
					return false;
				}
			}
		...
		private bool ValidatePathIsWriteable(GalleryServerPro.WebControls.wwDataBindingItem item, string fullPathToTest)
		{
