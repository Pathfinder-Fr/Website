...
namespace GalleryServerPro.Web.Pages.Admin
{
	public partial class usersettings : Pages.AdminPage
	{
		...
		public string DefaultRolesForSelfRegisteredUser
		{
			...
			set
			{
				string[] roleNames = value.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
		...
		private bool ValidateUsersToNotifyWhenAccountIsCreated(wwDataBindingItem item)
		{
			if ((item.ControlInstance == this.cboUsersToNotify))
			{
				...
				if ((!this.cboUsersToNotify.Text.Equals(userNamesCurrent)))
				{
					...
					{
						if (UserController.GetUser(Util.HtmlDecode(userName.Trim()), false) == null)
						{
		...
		private void ConfigureOrphanUserAlbums()
		{
			...
			{
				string userAlbumParentTitle = Util.RemoveHtmlTags(Factory.LoadAlbumInstance(GallerySettingsUpdateable.UserAlbumParentAlbumId, false).Title);

				if (orphanUserAlbums.Count > 1)
					lblOrphanUserAlbumsMsg.Text = String.Format(Resources.GalleryServerPro.Admin_User_Settings_Orphan_User_Albums_Many_Lbl, orphanUserAlbums.Count, userAlbumParentTitle);
				else
					lblOrphanUserAlbumsMsg.Text = String.Format(Resources.GalleryServerPro.Admin_User_Settings_Orphan_User_Albums_One_Lbl, orphanUserAlbums.Count, userAlbumParentTitle);
		...
		private List<IAlbum> GetOrphanUserAlbums()
		{
			...
			if (albumId > 0)
			{
				try
				{
					userAlbumParent = Factory.LoadAlbumInstance(albumId, false);
		...
		private void ConfigureUsersToNotifyComboBoxFirstTime()
		{
			...
			{
				userListItems.Add(new ListItem(Util.HtmlEncode(user.UserName), Util.HtmlEncode(user.UserName)));
		...
		private void BindDefaultRolesComboBox()
		{
			...
			{
				roleListItems.Add(new ListItem(RoleController.ParseRoleNameFromGspRoleName(Util.HtmlEncode(role.RoleName)), Util.HtmlEncode(role.RoleName)));
