...
namespace GalleryServerPro.Web.Pages.Admin
{
	public partial class backuprestore : Pages.AdminPage
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			{
				Util.Redirect(PageId.admin_gallerysettings, "aid={0}", this.GetAlbumId());
			}
		...
		private void ConfigureControlsEveryPageLoad()
		{
			...
			tsBackupRestore.ImagesBaseUrl = String.Concat(Util.GalleryRoot, "/images/componentart/tabstrip/");
			tsBackupRestore.TopGroupSeparatorImagesFolderUrl = String.Concat(Util.GalleryRoot, "/images/componentart/tabstrip/");
		...
		private void AddUploadControlForFullTrust()
		{
			...
			upload.FileInputImageUrl = Util.GetUrl("/images/componentart/upload/transparent.gif");
			upload.FileInputHoverImageUrl = Util.GetUrl("/images/componentart/upload/transparent.gif");
		...
		private string SaveFileToTempDirectory(FileUpload fileToRestore)
		{
			...
			if (Path.GetExtension(fileToRestore.FileName).Equals(".zip", StringComparison.OrdinalIgnoreCase))
			{
				ZipUtility zip = new ZipUtility(Util.UserName, GetGalleryServerRolesForUser());
		...
		private void ConfigureBackupFileInfo(IBackupFile backupFile)
		{
			...
			string[] tableNames = new string[]
				{
					"aspnet_Applications", "aspnet_Profile", "aspnet_Roles", "aspnet_Users", "aspnet_UsersInRoles", "gs_Gallery", "gs_Album", 
			...
			foreach (string tableName in tableNames)
			{
				switch (tableName)
				{
					...
					case "gs_Role":
						...
						break;
					case "gs_AppSetting":
						lblNumAppSettings.Text = (dataRecords.ContainsKey(tableName) ? backupFile.DataTableRecordCount[tableName].ToString() : String.Empty);
						break;
					case "gs_GalleryControlSetting":
						lblNumGalleryControlSettings.Text = (dataRecords.ContainsKey(tableName) ? backupFile.DataTableRecordCount[tableName].ToString() : String.Empty);
						break;
					case "gs_GallerySetting":
						lblNumGallerySettings.Text = (dataRecords.ContainsKey(tableName) ? backupFile.DataTableRecordCount[tableName].ToString() : String.Empty);
						break;
					case "gs_BrowserTemplate":
						lblNumBrowserTemplates.Text = (dataRecords.ContainsKey(tableName) ? backupFile.DataTableRecordCount[tableName].ToString() : String.Empty);
						break;
					case "gs_MimeType":
						lblNumMimeTypes.Text = (dataRecords.ContainsKey(tableName) ? backupFile.DataTableRecordCount[tableName].ToString() : String.Empty);
						break;
					case "gs_MimeTypeGallery":
						lblNumMimeTypeGalleries.Text = (dataRecords.ContainsKey(tableName) ? backupFile.DataTableRecordCount[tableName].ToString() : String.Empty);
						break;
					case "gs_AppError":
						lblNumAppErrors.Text = (dataRecords.ContainsKey(tableName) ? backupFile.DataTableRecordCount[tableName].ToString() : String.Empty);
						break;
					case "gs_UserGalleryProfile":
						lblNumUserGalleryProfiles.Text = (dataRecords.ContainsKey(tableName) ? backupFile.DataTableRecordCount[tableName].ToString() : String.Empty);
						break;
				}
			}
			...
			if (backupFile.IsValid)
			{
				...
				imgValidationResult.ImageUrl = Util.GetUrl("/images/info.gif");
			...
			{
				btnRestore.Enabled = false;
				imgValidationResult.ImageUrl = Util.GetUrl("/images/warning.gif");
				imgValidationResult.Visible = true;
