		...
		<div class="gsp_addleftpadding6">
			...
			<div class="gsp_addleftmargin10">
				...
				<asp:Panel ID="pnlOrphanUserAlbums" runat="server" Width="575" CssClass="gsp_p_a_us_oac gsp_rounded10"
					Visible="false">
					<img style="float: left; width: 32px; height: 32px;" class="gsp_p_a_us_ii" src="<%= Util.GetUrl("/images/info_32x32.png") %>"
						alt="" />
					<asp:Label ID="lblOrphanUserAlbumsMsg" runat="server" CssClass="gsp_p_a_us_orm" />
					<div class="gsp_p_a_us_doa">
						<img src="<%= Util.GetUrl("/images/componentart/grid/delete.png") %>" alt="" style="width: 16px;
					...
					<div class="gsp_p_a_us_t">
						<input id="gsp_orphan_ua_sa" type="checkbox" onclick="javascript:$('#<%= pnlOrphanUserAlbums.ClientID %> :input[type=checkbox]').attr('checked', this.checked);" /> <label for="gsp_orphan_ua_sa"><asp:Literal Text="<%$ Resources: GalleryServerPro, Admin_User_Settings_Orphan_User_Albums_Select_All_Lbl %>" runat="server" /></label>
					</div>
					<div id="gsp_chk_container" class="gsp_p_a_us_oal">
						<asp:Repeater ID="rptrOrphanUserAlbums" runat="server">
							<ItemTemplate>
								<p class="gsp_p_a_us_oa">
									<input type="checkbox" id='<%# "chk" + (Eval("ID")) %>' />&nbsp;<a href="<%= Util.GetCurrentPageUrl() %>?aid=<%# Eval("ID") %>" 
									title="<asp:Literal ID="l8" runat="server" Text="<%$ Resources:GalleryServerPro, Admin_User_Settings_View_Orphan_User_Album_Tooltip %>" />"><%# Util.RemoveHtmlTags(Eval("Title").ToString()) %></a>
								</p>
							</ItemTemplate>
						</asp:Repeater>
					</div>
