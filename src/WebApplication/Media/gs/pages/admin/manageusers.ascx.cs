...
namespace GalleryServerPro.Web.Pages.Admin
{
	public partial class manageusers : Pages.AdminPage
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			// Update 30 Jun 2010: The error happens if you move the ConfigureControls() function after the if (!IsPostBack) below; not sure why.
		...
		protected void cblAvailableRolesForExistingUser_DataBound(object sender, EventArgs e)
		{
			foreach (ListItem item in cblAvailableRolesForExistingUser.Items)
			{
				...
				item.Text = RoleController.ParseRoleNameFromGspRoleName(Util.HtmlEncode(item.Text));
		...
		protected void cblAvailableRolesForNewUser_DataBound(object sender, EventArgs e)
		{
			foreach (ListItem checkbox in cblAvailableRolesForNewUser.Items)
			{
				...
				// HTML encode the values.
				checkbox.Text = Util.HtmlEncode(roleName);
		...
		{
			cbEditUser.CallbackPrefix = Util.GetCurrentPageUri().ToString();
			cbAddUser.CallbackPrefix = Util.GetCurrentPageUri().ToString();
			...
			if (cbValidateNewUserName != null)
				cbValidateNewUserName.CallbackPrefix = Util.GetCurrentPageUri().ToString();
...
<p class=""gsp_textcenter""><img src=""{1}"" style=""width: 16px; height: 16px;""	alt="""" /></p>",
		...
		private void BindRolePermissionCheckboxes(IUserAccount user)
		{
			...
			{
				string checkboxRoleName = Util.HtmlDecode(checkbox.Text);
