...
namespace GalleryServerPro.Web.Pages.Admin
{
	public partial class manageroles : Pages.AdminPage
	{
		...
		{
			cbEditRole.CallbackPrefix = Util.GetCurrentPageUri().ToString();
		}
		...
		protected void cbEditRole_Callback(object sender, ComponentArt.Web.UI.CallBackEventArgs e)
		{
			...
			catch (Exception ex)
			{
				...
				msgBox.MessageTitle = e.Parameter.Equals("save") ? Resources.GalleryServerPro.Admin_Manage_Roles_Cannot_Save_Role_Msg : Resources.GalleryServerPro.Site_Error_Hdr;
...
<p class=""gsp_textcenter""><img src=""{1}"" style=""width: 16px; height: 16px;""	alt="""" /></p>",
	...
	Util.GetUrl("images/componentart/callback/spinner.gif"));
		...
		private void AddRoleNameColumnClientTemplate()
		{
			...
			roleNameColumn.Text = "## parseRoleNameFromGspRoleName(DataItem.getMember('RoleName').get_value()) ##";
		...
		private void PopulateControlsWithRoleData(string roleName)
		{
			...
			if (!String.IsNullOrEmpty(roleName))
			{
				...
				lblRoleName.Text = RoleController.ParseRoleNameFromGspRoleName(Util.HtmlEncode(role.RoleName));
			}
		...
		private void RegisterJavaScript()
		{
			...
			{{
				return confirm('{2} ' + parseRoleNameFromGspRoleName(roleName) + '?');
			}}
			...
			{{
				...
				return ($('#{3}').attr('checked') || $('#{18}').attr('checked'));
			}}
			...
			{{
				...
				if ($('#{3}').attr('checked')) return false;
