...
namespace GalleryServerPro.Web.Pages.Admin
{
	public partial class gallerycontrolsettings : Pages.AdminPage
	{
		...
		private void ConfigureControlsEveryTime()
		{
			...
			lblGalleryDescription.Text = String.Format(Resources.GalleryServerPro.Admin_Gallery_Control_Description_Label, GalleryControl.ClientID, Util.GetCurrentPageUrl());
		}
		...
		private void ConfigureControlsFirstTime()
		{
			...
			rbDefaultGallery.Text = String.Format(Resources.GalleryServerPro.Admin_Gallery_Control_Settings_Default_Gallery_Label,
																						Util.HtmlEncode(Factory.LoadGallery(GalleryId).Description));
		...
		private void DataBindDefaultGalleryObject()
		{
			...
			else if (GalleryControl.GalleryControlSettings.MediaObjectId.HasValue)
			{
				...
				txtDefaultMediaObjectId.Text = GalleryControl.GalleryControlSettings.MediaObjectId.Value.ToString();
			}
			else
			{
		...
		private void ConfigureDefaultAlbumComboBoxFirstTime()
		{
			...
			cboDefaultAlbum.DropHoverImageUrl = Util.GetUrl("/images/componentart/combobox/ddn-hover.png");
			cboDefaultAlbum.DropImageUrl = Util.GetUrl("/images/componentart/combobox/ddn.png");
			...
			if (albumId > 0)
			{
				try
				{
					IAlbum albumToSelect = Factory.LoadAlbumInstance(albumId, false);
		...
		private void SaveSettings()
		{
			...
			if (!UnbindError)
			{
				...
				Util.Redirect(PageId.admin_gallerycontrolsettings, "aid={0}&msg={1}", GetAlbumId(), ((int)msg).ToString(CultureInfo.InvariantCulture));
			}
