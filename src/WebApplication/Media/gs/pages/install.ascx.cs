...
";
		...
		/// <summary>
		/// Check web.config to make sure they contain references to the SQLite and SQL Server providers.
		...
		{
			if (WebConfigController.AreProvidersAvailableInWebConfig("SQLite"))
			{
				// Web.config contain references to SQLite providers, so it is a valid choice. Select it, unless one 
				...
				// by getting to this point, that if web.config contains references to SQLite, it is a valid choice.)
			...
			if (WebConfigController.AreProvidersAvailableInWebConfig("SqlServer"))
			{
		...
		private void UpdateWebConfigFile()
		{
			try
			{
				...
				if (!AreProvidersSpecifiedInWebConfig())
				{
					...
					webConfigEntity.MembershipDefaultProvider = (ProviderDb == ProviderDataStore.SQLite ? MembershipDataProvider.SQLiteMembershipProvider : MembershipDataProvider.SqlMembershipProvider);
					webConfigEntity.RoleDefaultProvider = (ProviderDb == ProviderDataStore.SQLite ? RoleDataProvider.SQLiteRoleProvider : RoleDataProvider.SqlRoleProvider);
					webConfigEntity.GalleryDataDefaultProvider = (ProviderDb == ProviderDataStore.SQLite ? GalleryDataProvider.SQLiteGalleryServerProProvider : GalleryDataProvider.SqlServerGalleryServerProProvider);
			...
			System.Web.UI.HtmlControls.HtmlGenericControl errorCallStackControl)
		{
			...
			if (errorSqlControl != null)
			{
				...
				{
					errorSqlControl.InnerHtml = String.Format(CultureInfo.CurrentCulture, "<span class='gsp_bold'>{0}</span> {1}", Resources.GalleryServerPro.Installer_Sql_Error_Msg, Util.HtmlEncode(sqlThatCausedError));
			...
			if (errorCallStackControl != null)
			{
				...
				{
					errorCallStackControl.InnerHtml = String.Format(CultureInfo.CurrentCulture, "<span class='gsp_bold'>{0}</span> {1}", Resources.GalleryServerPro.Installer_Sql_Error_CallStack_Label, Util.HtmlEncode(callStack));
		...
		/// <summary>
		/// Verify the SQLite or SQL Server providers are configured in web.config. The requested provider does not have to be specified as the 
		...
		private bool AreProvidersSpecifiedInWebConfig()
		{
			...
			MembershipDataProvider membershipProviderName = (ProviderDb == ProviderDataStore.SQLite ? MembershipDataProvider.SQLiteMembershipProvider : MembershipDataProvider.SqlMembershipProvider);
			RoleDataProvider roleProviderName = (ProviderDb == ProviderDataStore.SQLite ? RoleDataProvider.SQLiteRoleProvider : RoleDataProvider.SqlRoleProvider);
			GalleryDataProvider galleryDataProviderName = (ProviderDb == ProviderDataStore.SQLite ? GalleryDataProvider.SQLiteGalleryServerProProvider : GalleryDataProvider.SqlServerGalleryServerProProvider);
		...
		private void DeleteGalleryServerProConfigFile()
		{
			...
			try
			{
				File.Delete(Server.MapPath(Util.GetUrl("/config/galleryserverpro.config")));
			}
			catch (IOException) { }
			catch (UnauthorizedAccessException) { }
			catch (System.Security.SecurityException) { }
