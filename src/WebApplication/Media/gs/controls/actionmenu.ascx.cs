...
namespace GalleryServerPro.Web.Controls
{
	public partial class actionmenu : GalleryUserControl
	{
		...
		private void SetMenuItemsText()
		{
            ...
            // Sueetie Modified - Include In Download Menuitem
            mnuIncludeInDownload.Text = Resources.Sueetie.UC_ActionMenu_Include_Download_Text;
            mnuEditSueetieCaptions.Text = Resources.Sueetie.UC_ActionMenu_EditSueetieCaptions_Text;
            mnuEditSueetieAlbums.Text = Resources.Sueetie.UC_ActionMenu_EditSueetieAlbums_Text;
		}
		...
		private void SetUrlTooltips()
		{
            ...
            // Sueetie Modified - Include In Download Menuitem
            mnuIncludeInDownload.ToolTip = Resources.Sueetie.UC_ActionMenu_Include_Download_Tip;
            mnuEditSueetieCaptions.ToolTip = Resources.Sueetie.UC_ActionMenu_EditSueetieCaptions_Tip;
            mnuEditSueetieAlbums.ToolTip = Resources.Sueetie.UC_ActionMenu_EditSueetieAlbums_Tip;
		...
		private void SetNavigationUrls()
		{
			...
			mnuSiteSettings.NavigateUrl = Util.GetUrl(PageId.admin_sitesettings, "aid={0}", this.AlbumId);
			//mnuLogout.NavigateUrl = // No navigation URL. Instead we have an event handler (Menu1_ItemSelected)

            // Sueetie Modified - Include In Download Menuitem
            mnuIncludeInDownload.NavigateUrl = Util.GetUrl(PageId.task_includeindownload, "aid={0}", this.AlbumId);
            mnuEditSueetieCaptions.NavigateUrl = Util.GetUrl(PageId.task_editsueetiecaptions, "aid={0}", this.AlbumId);
            mnuEditSueetieAlbums.NavigateUrl = Util.GetUrl(PageId.task_editsueetiealbums, "aid={0}", this.AlbumId);
		...
		private void EnableOrDisableMenuItemsBasedOnUserPermission()
		{
            ...
            // Sueetie Modified - Include In Download Menuitem
            mnuIncludeInDownload.Enabled = (!this.GalleryPage.GetAlbum().IsRootAlbum && this.GalleryPage.UserCanDeleteMediaObject);
            ...
            // Sueetie Modified - Include In Download Menuitem
            if (!mnuIncludeInDownload.Enabled)
                mnuIncludeInDownload.ToolTip = noPermissionTooltip;
            if (!mnuEditSueetieCaptions.Enabled)
                mnuIncludeInDownload.ToolTip = noPermissionTooltip;
            if (!mnuEditSueetieAlbums.Enabled)
                mnuIncludeInDownload.ToolTip = noPermissionTooltip;
		...
		private void EnableOrDisableMenuItemsBasedOnWriteAccess()
		{
			...
			if (isReadOnly)
			{
                ...
                // Sueetie Modified - Include In Download Menuitem
                mnuIncludeInDownload.Enabled = false;
                mnuIncludeInDownload.ToolTip = Resources.GalleryServerPro.UC_ActionMenu_Disabled_ReadOnly_Tooltip;
			}
		}
		...
		private void ReloadPage()
		{
			...
			if (this.GalleryPage.GetMediaObjectId() > int.MinValue)
			{
				...
				url = Util.AddQueryStringParameter(url, String.Concat("moid=", this.GalleryPage.GetMediaObjectId()));
				Util.Redirect(url);
			}

			// If not looking at a media object, redirect to current album.
			Util.Redirect(PageId.album, "aid={0}", this.GalleryPage.GetAlbumId());
		}
