...
namespace GalleryServerPro.Web.Controls
{
	public partial class mediaobjectview : GalleryUserControl
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			{
				Util.Redirect(Util.AddQueryStringParameter(Util.GetCurrentPageUrl(), "msg=" + (int)Message.MediaObjectDoesNotExist));
			}
		...
		private void TransmitMediaObjectWithWatermark(string filepath)
		{
			...
			try
			{
				watermarkedImage = GalleryServerPro.Business.ImageHelper.AddWatermark(filepath, GalleryPage.GalleryId);
	...
	{13}		
			...
			",
				...
				Util.GetUrl(PageId.album, "aid={0}", this.GalleryPage.GetAlbumId()), // 7
...
";
		...
		private string GetAnimationScript()
		{
			...
			switch (GalleryPage.GallerySettings.MediaObjectTransitionType)
			{
				case MediaObjectTransitionType.Fade:
					{
						// Note: More recent versions of the Ajax Toolkit have a different namespace, so you might need to use this syntax:
...
",
		...
		{
			Util.Redirect(PageId.task_downloadobjects, "aid={0}", this.GalleryPage.GetAlbumId());
		}

		private void RedirectToRotatePage()
		{
			Util.Redirect(PageId.task_rotateimage, "moid={0}", this.GalleryPage.GetMediaObjectId());
		}

		private void RedirectToMovePage()
		{
			Util.Redirect(PageId.task_transferobject, "moid={0}&tt=move&skipstep1=true", this.GalleryPage.GetMediaObjectId());
		}

		private void RedirectToCopyPage()
		{
			Util.Redirect(PageId.task_transferobject, "moid={0}&tt=copy&skipstep1=true", this.GalleryPage.GetMediaObjectId());
		}
		...
		private void ConfigureToolbar()
		{
            ...
            // Sueetie Modified - Show Only Slideshow button when IsMobile
