...
<p class="gsp_searchlink gsp_useroption">
	<a id="l1" href="javascript: toggleSearch();" class="gsp_addrightmargin2" title='<asp:Literal ID="l3" runat="server" Text="<%$ Resources:GalleryServerPro, Search_Button_Tooltip %>" />'>
		<img src="<%= Util.GalleryRoot %>/images/search_32x32.png" style="width:32px;height:32px;" alt='<asp:Literal
	...
	<ContentTemplate>
		<asp:Panel ID="pnlSearch" runat="server" DefaultButton="btnSearch" CssClass="gsp_search">
			<p class="gsp_textright">
				<img onclick="dgSearch.Close('cancelled');" src="<%= Util.GalleryRoot %>/images/componentart/dialog/close.gif"
					alt="Close" /></p>
			<img src="<%= Util.GalleryRoot %>/images/search_48x48.png" class="gsp_searchicon" alt="" style="width:48px;height:48px;" />
			<p>
