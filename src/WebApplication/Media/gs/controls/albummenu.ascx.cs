	...
	public partial class albummenu : GalleryUserControl
	{
		...
		private string BuildMenuString()
		{
			...
			while (!foundTopAlbum)
			{
				...
				else if (!Util.IsUserAuthorized(SecurityActions.ViewAlbumOrMediaObject, roles, album.Id, album.GalleryId, album.IsPrivate))
				{
					...
					IAlbum rootAlbum = Factory.LoadRootAlbum(this.GalleryPage.GalleryId, SecurityActions.ViewAlbumOrMediaObject | SecurityActions.ViewOriginalImage, GalleryPage.GetGalleryServerRolesForUser(), Util.IsAuthenticated);
				...
				else
				{
                    ...
                    if (renderLinks)
                    {
                        // Sueetie Modified - Display "All Albums" for Recent Photos
                        if (DataHelper.GetIntFromQueryString("aid", 1) == -1)
                            menuString = menuString.Insert(0, string.Format(CultureInfo.CurrentCulture, " {0} <a{1} href=\"{2}?aid=-1\">{3}</a>", dividerText, hyperlinkIdString, appPath, Resources.Sueetie.Menulink_RecentPhotos));
                        else
                    ...
                    {
                        menuString = menuString.Insert(0, string.Format(CultureInfo.CurrentCulture, " {0} {1}", dividerText, Util.RemoveHtmlTags(album.Title)));
                    }

                    // Sueetie Modified - Display "All Albums" for Recent Photos
                    if (DataHelper.GetIntFromQueryString("aid", 1) == -1)
                    {
                        IAlbum _topAlbum = Factory.LoadAlbumInstance(1, false);
                        menuString = menuString.Insert(0, string.Format(CultureInfo.CurrentCulture, " {0} <a href=\"{1}?aid=1\">{2}</a>", dividerText, appPath, Resources.GalleryServerPro.Site_Virtual_Album_Title));
                    }
				}
