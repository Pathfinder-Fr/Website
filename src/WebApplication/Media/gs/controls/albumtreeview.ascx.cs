...
namespace GalleryServerPro.Web.Controls
{
	public partial class albumtreeview : GalleryUserControl
	{
		...
		private IEnumerable<IAlbum> GetRootAlbums()
		{
			...
			{
				rootAlbums.Add(Factory.LoadAlbumInstance(RootAlbumId, true));
			}
			else
			{
				foreach (IGallery gallery in Galleries)
				{
					rootAlbums.Add(Factory.LoadRootAlbum(gallery.GalleryId, this.RequiredSecurityPermissions, this.GalleryPage.GetGalleryServerRolesForUser(), Util.IsAuthenticated));
				}
			}
		...
		private void BindAlbumToTreeview(IGalleryObjectCollection albums, TreeViewNode parentNode, bool expandNode)
		{
			...
			foreach (IAlbum album in albums)
			{
				...
				string albumTitle = Util.RemoveHtmlTags(album.Title);
				...
				if (!String.IsNullOrEmpty(NavigateUrl))
				{
					node.NavigateUrl = String.Format("{0}?aid={1}", NavigateUrl, album.Id);
				...
				{
					node.ContentCallbackUrl = String.Format(CultureInfo.CurrentCulture, "{0}?aid={1}{2}&sc={3}&nurl={4}", handlerPath, album.Id.ToString(CultureInfo.InvariantCulture), this.SecurityPermissionQueryStringParm, node.ShowCheckBox, Util.HtmlEncode(NavigateUrl));
				}
