...
namespace GalleryServerPro.Web
{
	...
	public static class Util
	{
		...
		{
			return string.Concat(Util.GetCurrentPageUrl(), "?g=", page);
		}
		...
		public static string GetUrl(PageId page, string format, params object[] args)
		{
			if ((page == PageId.album) || (page == PageId.mediaobject))
			{
				...
				return string.Concat(Util.GetCurrentPageUrl(), "?", String.Format(format, args));
			}
			else
				return string.Concat(Util.GetCurrentPageUrl(), "?g=", page, "&", String.Format(format, args));
		}
		...
		public static Uri GetCurrentPageUri()
		{
			...
			uriBuilder.Host = Util.GetHostName();
		...
		/// This method is a wrapper for a call to HttpContext.Current.Request.RawUrl. Example: /dev/gs/gallery.aspx
		/// </summary>
		...
		/// This method is a wrapper for a call to HttpContext.Current.Request.Url.RawUrl. Example: /dev/gs/gallery.aspx?g=admin_email&amp;aid=2389.
		...
		public static string GetCurrentPageUrl(bool includeQueryString)
		{
			...
			if (includeQueryString)
				...
				return RemoveQueryStringParameter(HttpContext.Current.Request.RawUrl);
		}
		...
		public static string GetHostUrl()
		{
			...
			string hostUrl = (String)HttpContext.Current.Session["HostUrl"];
		...
		public static string GetHostName()
		{
			...
			return (host.IndexOf(":") < 0 ? host : host.Substring(0, host.IndexOf(":")));
		}
		...
		public static void PerformMaintenance()
		{
			...
			if (mustRunMaintenance)
			{
				try
				{
					...
					// Update albums to clear out the OwnerUserName property when the user does not exist in the current membership.
					UserController.DeleteOrphanedAlbumOwners();
		...
		/// <summary>
		/// Determines whether the <see cref="url" /> is an absolute URL rather than a relative one. An URL is considered absolute if
		/// it starts with "http" or "//".
		/// </summary>
		/// <param name="url">The URL to check.</param>
		/// <returns>
		/// 	<c>true</c> if the <see cref="url" /> is absolute; otherwise, <c>false</c>.
		/// </returns>
		public static bool IsAbsoluteUrl(string url)
		{
			return (url.StartsWith("http", true, CultureInfo.InvariantCulture) || url.StartsWith("//", true, CultureInfo.InvariantCulture));
		}
