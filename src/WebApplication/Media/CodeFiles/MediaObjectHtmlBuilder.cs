...
namespace GalleryServerPro.Web
{
	...
	public class MediaObjectHtmlBuilder : IMediaObjectHtmlBuilder
	{
		...
		public string GenerateScript()
		{
			...
			scriptOutput = scriptOutput.Replace("{HostUrl}", Util.GetHostUrl());
			...
			scriptOutput = scriptOutput.Replace("{TitleNoHtml}", Util.RemoveHtmlTags(this.Title, true));
			string[] cssClasses = new string[this.CssClasses.Count];
			this.CssClasses.CopyTo(cssClasses, 0);
			...
			if (scriptOutput.Contains("{GalleryPath}"))
				scriptOutput = scriptOutput.Replace("{GalleryPath}", Util.GalleryRoot);
		...
		private string ReplaceMediaObjectAbsoluteUrlNoHandlerParameter(string htmlOutput)
		{
			...
			relativePath = Util.UrlEncode(relativePath, '\\');

			string directUrl = String.Concat(Util.UrlEncode(Util.AppRoot, '/'), "/", relativePath.Replace("\\", "/"));
