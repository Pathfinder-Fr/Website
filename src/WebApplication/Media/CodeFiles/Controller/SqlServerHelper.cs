...
namespace GalleryServerPro.Web.Sql
{
	public class SqlServerHelper
	{
		...
		private static string GetSqlPath(string sqlScriptName)
		{
			...
			string filePath = HttpContext.Current.Server.MapPath(Util.GetUrl(String.Concat(sqlPath, sqlScriptName)));

			if (!File.Exists(filePath))
			{
				throw new FileNotFoundException(string.Format("Could not find the file \"{0}\".", filePath), sqlScriptName);
			}
				...
				using (SqlConnection cn = new SqlConnection(_connectionString))
				{
					...
					while (!sr.EndOfStream)
					{
						...
						using (SqlCommand cmd = cn.CreateCommand())
						{
							...
							{
								sb.Replace("{galleryId}", _galleryId.Value.ToString());
							}
