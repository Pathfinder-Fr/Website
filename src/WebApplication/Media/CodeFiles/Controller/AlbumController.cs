...
namespace GalleryServerPro.Web.Controller
{
	...
	public static class AlbumController
	{
		...
		public static IAlbum CreateUserAlbum(string userName, int galleryId)
		{
			...
			try
			{
				parentAlbum = Factory.LoadAlbumInstance(gallerySetting.UserAlbumParentAlbumId, false);
			}
			catch (ErrorHandler.CustomExceptions.InvalidAlbumException ex)
			{
				...
				string galleryDescription = Util.HtmlEncode(Factory.LoadGallery(gallerySetting.GalleryId).Description);
				...
				AppErrorController.LogError(new ErrorHandler.CustomExceptions.WebException(msg, ex), galleryId);
				return null;
			}
		...
		public static void ValidateBeforeAlbumDelete(IAlbum albumToDelete)
		{
			...
			if (!validator.CanBeDeleted)
			{
				switch (validator.ValidationFailureReason)
				{
					...
					default:
						throw new System.ComponentModel.InvalidEnumArgumentException(string.Format("The function ValidateBeforeAlbumDelete is not designed to handle the enumeration value {0}. The function must be updated.", validator.ValidationFailureReason));
				}
			}
		}
