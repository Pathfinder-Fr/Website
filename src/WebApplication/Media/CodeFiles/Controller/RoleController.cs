...
namespace GalleryServerPro.Web.Controller
{
	...
	public static class RoleController
	{
		...
		{
			return GetGalleryServerRolesForUser(Util.UserName);
		}
		...
		{
			return roleName.Trim().StartsWith(GlobalConstants.AlbumOwnerRoleNamePrefix);
		}
		...
		public static void RemoveRolesFromCache()
		{
			...
			{
				rolesCache.Remove(GetCacheKeyNameForRoles(Util.UserName));
			}
		}
		...
		private static void ValidateCanRemoveSiteAdminPermission(IGalleryServerRole roleToSave, IGalleryServerRole existingRole)
		{
			...
			{
				throw new ArgumentOutOfRangeException(String.Format("The role name of the roleToSave and existingRole parameters must match, but they do not. roleToSave='{0}'; existingRole='{1}'", roleToSave, existingRole));
			}
		...
		private static void ValidateGalleryAdminCanSaveRole(IGalleryServerRole roleToSave, IGalleryServerRole existingRole)
		{
			...
			if (!Util.IsUserSiteAdministrator(roles))
			{
				...
				{
					if (!Util.IsUserGalleryAdministrator(roles, gallery.GalleryId))
					{
		...
		private static void ValidateUserDoesNotLoseAbilityToAdminCurrentGallery(IGalleryServerRole roleToSave, IGalleryServerRole existingRole)
		{
			...
			if (IsUserInRole(Util.UserName, roleToSave.RoleName))
			{
		...
		private static void ValidatePreventLoggedOnUserFromLosingAdminAccess(IGalleryServerRole roleToDelete)
		{
			...
			if (roleToDelete.AllowAdministerSite || roleToDelete.AllowAdministerGallery)
			{
				...
				IGalleryServerRoleCollection roles = GetGalleryServerRolesForUser(Util.UserName);
				foreach (IGalleryServerRole role in roles)
				{
		...
		private static void ValidatePreventRoleDeletionAffectingOtherGalleries(IGalleryServerRole roleToDelete)
		{
			...
			foreach (IGallery gallery in roleToDelete.Galleries)
			{
				...
				{
					throw new GallerySecurityException(String.Format(Resources.GalleryServerPro.Admin_Cannot_Delete_Role_Insufficient_Permission_Msg, roleToDelete.RoleName, gallery.Description));
				}
			}
		}
		...
		private static void UpdateAlbumOwnerBeforeRoleDelete(IGalleryServerRole role)
		{
			...
			{
				IAlbum album = Factory.LoadAlbumInstance(albumId, false, true);
		...
		private static string GenerateAlbumOwnerRoleName(IAlbum album)
		{
			...
			string roleNameTemplate = MakeRoleNameUnique(String.Format("{0} - {{UserName}} - {{AlbumTitle}} (album {1})", GlobalConstants.AlbumOwnerRoleNamePrefix, album.Id));
			...
			if (roleName.Length > maxRoleNameLength)
			{
				...
				if (numCharsTrimmed < numCharsToTrim)
				{
					...
					else
					{
						...
						throw new WebException(string.Format("Invalid role name length. Unable to shorten the album owner role name enough to satisfy maximum length restriction. Proposed name='{0}' (length={1}); Max length={2}", roleName, roleName.Length, maxRoleNameLength));
					}
				}
				...
				if (roleName.Length > maxRoleNameLength)
				{
					throw new WebException(string.Format("Unable to shorten the album owner role name enough to satisfy maximum length restriction. Proposed name='{0}' (length={1}); Max length={2}", roleName, roleName.Length, maxRoleNameLength));
				}
			}
		...
		private static bool IsUserGalleryAdminForRole(IGalleryServerRole role)
		{
			foreach (IGallery gallery in role.Galleries)
			{
				...
				if (Util.IsUserAuthorized(SecurityActions.AdministerGallery, rootAlbum.Id, gallery.GalleryId, rootAlbum.IsPrivate))
				{
					return true;
				}
			}
		...
		private static bool IsUserGalleryAdminForAlbumOwnerTemplateRole(IGalleryServerRole role)
		{
			...
			if (match.Success)
			{
				...
				int galleryId = Convert.ToInt32(match.Groups["galleryId"].Value);
