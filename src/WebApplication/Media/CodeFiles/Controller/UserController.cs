...
namespace GalleryServerPro.Web.Controller
{
	...
	public static class UserController
	{
		...
		{
			return ToUserAccount(MembershipGsp.GetUser(Util.UserName, false));
		}
		...
		{
			return GetUsersCurrentUserCanView(Util.IsCurrentUserSiteAdministrator(), Util.IsCurrentUserGalleryAdministrator(galleryId));
		}
		...
		public static IUserAccountCollection GetUsersCurrentUserCanView(bool userIsSiteAdmin, bool userIsGalleryAdmin)
		{
			...
			else if (userIsGalleryAdmin)
			{
				...
				{
					cacheKeyName = GetCacheKeyNameForUsersCurrentUserCanView(Util.UserName);
		...
		{
			return GetUserAlbum(Util.UserName, galleryId);
		}
		...
		public static IAlbum ValidateUserAlbum(string userName, int galleryId)
		{
			...
			if (galleryId == Int32.MinValue)
			{
				...
				throw new ArgumentOutOfRangeException(String.Format("A valid gallery ID must be passed to the UserController.ValidateUserAlbum function when user albums are enabled. Instead, the value {0} was passed for the gallery ID.", galleryId));
			}
			...
			if (albumId > Int32.MinValue)
			{
				try
				{
					...
					album = Factory.LoadAlbumInstance(albumId, false, true);
		...
		/// <summary>
		/// Updates albums to clear out the <see cref="IAlbum.OwnerUserName" /> property when the user does not exist in the current membership.
		/// This function is resource intensive and is intended to be called periodically; for example, once during each application restart.
		/// </summary>
		public static void DeleteOrphanedAlbumOwners()
		{
			IUserAccountCollection allUsers = GetAllUsers();
			List<string> userNames = new List<string>(allUsers.Count);

			foreach (IUserAccount user in allUsers)
			{
			...
			foreach (IGallery gallery in Factory.LoadGalleries())
			{
				DeleteOrphanedAlbumOwner(Factory.LoadRootAlbumInstance(gallery.GalleryId, false), userNames);
			}
		}
		...
		{
			return GetGalleriesUserCanAdminister(Util.UserName);
		}
		...
		public static void ValidateLoggedOnUserHasPermissionToSaveUser(IUserAccount userToSave, string[] rolesToAdd, string[] rolesToRemove)
		{
			...
			if (Util.IsCurrentUserSiteAdministrator())
				return;
		...
		{
			if (!Util.IsCurrentUserSiteAdministrator() && !Util.IsCurrentUserGalleryAdministrator(galleryId))
			{
				throw new GallerySecurityException("You must be a gallery or site administrator to save changes to this user.");
			}

			if (userToSave.UserName.Equals(Util.UserName, StringComparison.OrdinalIgnoreCase))
			{
			...
			foreach (string roleName in rolesBeingAddedOrRemoved)
			{
				...
				if (roleToAddOrRemove != null)
				{
					foreach (IGallery gallery in roleToAddOrRemove.Galleries)
					{
						if (!adminGalleries.Contains(gallery))
						{
							throw new GallerySecurityException(string.Format("You are attempting to save changes to a user that will affect multiple galleries, including at least one gallery you do not have permission to administer. Specifically, the role '{0}' applies to gallery {1}, which you are not an administrator for.", roleToAddOrRemove.RoleName, gallery.GalleryId));
						}
					}
				}
			}
