    ...
    public partial class Search : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                var selectedCategories = new List<string>(allCategories.Count);

                // Populate categories list and select specified categories if any
                foreach (CategoryInfo cat in allCategories)
                {
                    var item = new ListItem(NameTools.GetLocalName(cat.FullName), cat.FullName);
        ...
        protected void btnGo_Click(object sender, EventArgs e)
        {
            ...
            var query = ScrewTurn.Wiki.SearchEngine.Tools.RemoveDiacriticsAndPunctuation(txtQuery.Text, false);
...
<OpenSearchDescription xmlns=""http://a9.com/-/spec/opensearch/1.1/"">
    ...
    public class SearchResultRow
    {
        ...
        private static string GetExcerpt(PageInfo page, WordInfoCollection matches)
        {
            ...
            var sortedMatches = new List<WordInfo>(matches);
        ...
        private static string BuildFormattedExcerpt(List<WordInfo> matches, string input)
        {
            ...
            var shift = 100;
            var maxLen = 600;
            var highlightOpen = "<b class=\"searchkeyword\">";
            var highlightClose = "</b>";
            ...
            for (var i = 0; i < matches.Count; i++)
            {
                ...
                var openIndex = match.FirstCharIndex + i * (highlightOpen.Length + highlightClose.Length);
                var openIndexOk = openIndex >= 0 && openIndex <= sb.Length;
                if (openIndexOk) sb.Insert(openIndex, highlightOpen);

                var closeIndex = match.FirstCharIndex + match.Text.Length + highlightOpen.Length + i * (highlightOpen.Length + highlightClose.Length);
            ...
            if (matches.Count > 0)
            {
                var start = matches[0].FirstCharIndex - shift;
                if (start < 0)
                {
                    start = 0;
                    startsAtZero = true;
                }
                var len = matches[matches.Count - 1].FirstCharIndex + matches[matches.Count - 1].Text.Length + shift - matches.Count * (highlightOpen.Length + highlightClose.Length) - start;
