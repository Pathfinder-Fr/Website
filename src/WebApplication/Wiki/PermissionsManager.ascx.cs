...
namespace ScrewTurn.Wiki {
	...
	public partial class PermissionsManager : System.Web.UI.UserControl {
		...
		protected void lstSubjects_SelectedIndexChanged(object sender, EventArgs e) {
            ...
            {
                var subject = selectedSubject.Substring(2);
                var type = selectedSubject.StartsWith("G.") ? SubjectType.Group : SubjectType.User;

                try
                {
                    DisplaySubjectPermissions(subject, type);
                }
                catch (Exception ex)
                {
                    throw new InvalidOperationException(string.Format("Unable to get permissions for subject {0}", selectedSubject), ex);
                }
            }
		}
