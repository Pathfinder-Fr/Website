...
namespace ScrewTurn.Wiki
{
    ...
    public partial class DefaultPage : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            var canView = AuthChecker.CheckActionForPage(currentPage, Actions.ForPages.ReadPage, currentUsername, currentGroups);
            var canEdit = false;
            ...
            // Sueetie Modified - Grab Current Sueetie UserName to fix reload bug for "Edit" button
            currentUsername = CurrentSueetieUser.UserName;

            var canDownloadAttachments = AuthChecker.CheckActionForPage(currentPage, Actions.ForPages.DownloadAttachments, currentUsername, currentGroups);
            var canSetPerms = AuthChecker.CheckActionForGlobals(Actions.ForGlobals.ManagePermissions, currentUsername, currentGroups);
            var canAdmin = AuthChecker.CheckActionForPage(currentPage, Actions.ForPages.ManagePage, currentUsername, currentGroups);
            var canViewDiscussion = AuthChecker.CheckActionForPage(currentPage, Actions.ForPages.ReadDiscussion, currentUsername, currentGroups);
            var canPostDiscussion = AuthChecker.CheckActionForPage(currentPage, Actions.ForPages.PostDiscussion, currentUsername, currentGroups);
            var canManageDiscussion = AuthChecker.CheckActionForPage(currentPage, Actions.ForPages.ManageDiscussion, currentUsername, currentGroups);
            ...
            // Sueetie Modified - Hardwiring "Edit" button to not require page reload
        ...
        {
            var title = FormattingPipeline.PrepareTitle(currentContent.Title, false, FormattingContext.PageContent, currentPage);
        ...
        private int GetAttachmentCount()
        {
            ...
            {
                count += prov.ListPageAttachments(currentPage).Count;
            }
            return count;
        }
        ...
        {
            var categories = Pages.GetCategoriesForPage(currentPage);
            ...
            {
                var sb = new StringBuilder(categories.Count * 10);
        ...
        private void SetupMetaInformation()
        {
            ...
            if (currentContent.Keywords != null && currentContent.Keywords.Count > 0)
            {
                var lit = new Literal();
        ...
        private string PrintKeywords(IEnumerable<string> keywords)
        {
        ...
        private void SetupBreadcrumbsTrail()
        {
            ...
            var pageTrail = SessionFacade.Breadcrumbs.LoadAllPages();
            var min = 3;
            ...
            for (var i = pageTrail.Count - min; i < pageTrail.Count; i++)
            {
            ...
            sb.Append(@""">[X]</a> ");
            for (var i = 0; i < pageTrail.Count; i++)
            {
        ...
        {
            var paths = NavigationPaths.PathsPerPage(currentPage);

            var currentPath = Request["NavPath"];
        ...
        private void SetupAdjacentPages()
        {
            ...
            if (Request["NavPath"] != null)
            {
                ...
                if (path != null)
                {
                    var idx = Array.IndexOf(path.Pages, currentPage.FullName);
                    if (idx != -1)
                    {
                        if (idx > 0)
                        {
        ...
        private void SetupEmailNotification()
        {
            ...
            {
                var pageChanges = false;
                var discussionMessages = false;
        ...
        private void SetupPageContent(bool canPostMessages, bool canManageDiscussion)
        {
            if (!discussMode && !viewCodeMode)
            {
                ...
                var html = Content.GetFormattedPageContent(currentPage, true);

                // Nettoyage HTML
                //var sanitizer = new Html.HtmlSanitizer();
                //sanitizer.AllowedAttributes = sanitizer.AllowedAttributes.Concat(new[] { "class" }).ToList();
            ...
            else if (!discussMode && viewCodeMode)
            {
                ...
                {
                    var literal = new Literal();
                    var sb = new StringBuilder(currentContent.Content.Length + 100);
            ...
            {
                var discussion = LoadControl("~/PageDiscussion.ascx") as PageDiscussion;
