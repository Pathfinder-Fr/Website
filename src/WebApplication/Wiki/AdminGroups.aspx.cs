...
namespace ScrewTurn.Wiki
{
    ...
    public partial class AdminGroups : BasePage
    {
        ...
        protected void rptGroups_DataBinding(object sender, EventArgs e)
        {
            ...
            var result = new List<UserGroupRow>(allGroups.Count);
        ...
        protected void btnCreate_Click(object sender, EventArgs e)
        {
            ...
            var done = Users.AddUserGroup(txtName.Text, txtDescription.Text,
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            var done = RemoveAllAclEntries(currentGroup);
            if (done)
            {
        ...
        {
            foreach (var action in grants)
            {
                var done = AuthWriter.SetPermissionForGlobals(AuthStatus.Grant, action, group);
                if (!done) return false;
            }

            foreach (var action in denials)
            {
                var done = AuthWriter.SetPermissionForGlobals(AuthStatus.Deny, action, group);
                if (!done) return false;
            }
